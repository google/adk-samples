...
| `Package 'retail-product-search' requires a different Python: 3.9.X not in '>=3.10'` | venv was created with system Python 3.9 (macOS default) | Recreate the venv with a 3.10+ interpreter: `rm -rf .venv && python3.12 -m venv .venv && source .venv/bin/activate && bash -c "pip install -e '${SKILL_DIR}[adk]'"` (substitute the Python version you have installed). Use the interpreter-detection loop from the Workspace Setup section to auto-find one. |
| `ModuleNotFoundError: google.adk` | Optional ADK dep missing | Use the standalone test path in Step 9, or `pip install -e '.[adk]'` from the skill dir (single-quote the `.[adk]` so zsh doesn't glob-expand the brackets) |
