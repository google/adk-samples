...
from _setup_utils import load_config  # noqa: E402  # pylint: disable=wrong-import-position
...
def delete_bigquery(project_id: str, dataset_id: str, dry_run: bool) -> bool:
    ...
    if dry_run:
        logger.info(
            "[DRY RUN] Would delete BigQuery dataset: %s (and all tables)", dataset_ref
        )
        return True
    ...
    for _ in range(max_iterations):
        request = vectorsearch.SearchDataObjectsRequest(
            ...
            semantic_search=vectorsearch.SemanticSearch(
                ...
                top_k=batch_size,
                output_fields=vectorsearch.OutputFields(data_fields=["product_id"]),
            ),
        )
        ...
        delete_requests = [
            vectorsearch.DeleteDataObjectRequest(name=name) for name in batch_ids
        ]
        ...
        logger.info("  Drained %d data objects (%d total)", len(batch_ids), deleted_total)
...
def delete_vectorsearch_collection(
    ...
    try:
        ...
        logger.info(
            "Vector Search collection %s does not exist, skipping", collection_id
        )
        return True
        ...
        logger.info(
            "Vector Search not available in %s; no collection to clean up.", location
        )
        return True
    ...
    try:
        logger.info("Draining data objects from %s before deletion...", collection_id)
...
def delete_vectorsearch_v1_index(
    ...
    for index in indexes:
        ...
        try:
            ...
            logger.info("Deleted legacy Vector Search index: %s", index.resource_name)
        except Exception as e:  # pylint: disable=broad-exception-caught
            logger.error(
...
def delete_cloudrun(
    ...
    if result.returncode != 0:
        logger.info("Cloud Run service %s does not exist, skipping", service_name)
        return True

    if dry_run:
        logger.info("[DRY RUN] Would delete Cloud Run service: %s", service_name)
        return True
    ...
    try:
        ...
        logger.error("Failed to delete Cloud Run service %s: %s", service_name, e)
        return False
...
def cleanup(
    ...
    collection_id = config.get("collection_id", "retail-skill-products-collection")
    ...
    if failures:
        logger.error(
            "Cleanup finished with errors. Failed steps: %s", ", ".join(failures)
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(Cloud Run excluded by default; opt in with --only cloudrun if you deployed to Cloud Run)."  # noqa: E501
        ),
    )
    parser.add_argument(
        "--dataset-id",
