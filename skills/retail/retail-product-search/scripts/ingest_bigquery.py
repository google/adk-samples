...
def _validate_and_convert(
    ...
    if all_errors:
        logger.warning("Skipped rows with %d validation errors:", len(all_errors))
        ...
        if if_exists == "skip":
            try:
                ...
                if row_count > 0:
                    ...
                    return False
                logger.info("Table %s exists but is empty -- loading rows.", table_ref)
            except Exception:  # pylint: disable=broad-exception-caught
                logger.info(
...
def ingest(
    ...
    logger.info("Successfully ingested %d products to %s", len(products), table_ref)
