...
def fetch_products(
    ...
    logger.info("Fetching products from %s.%s.%s", project_id, dataset_id, table_id)
...
def create_collection_if_needed(  # pylint: disable=too-many-arguments
    ...
    logger.info(
        "Creating collection '%s' with text_template: %s", collection_id, text_template
    )
    ...
    try:
        ...
        if not collection_id.islower() or any(c.isupper() for c in collection_id):
            hints.append(
