...
in a per-user Agent Runtime container instead of on the host.
...
| `401`/`403` from the sandbox shim | Per-user JWT + routing token at the Agent Runtime LB (`horizon/sandbox/lifecycle.py`, `environment.py`) | Bearer JWT or `X-Sandbox-Routing-Token` expired (env re-mints next turn), or the caller's ADC lacks `roles/iam.serviceAccountTokenCreator`. |
...
LLM's own behavior (jailbreaks), Agent Runtime / Cloud Run / IAP
...
| TB3 | Sandbox shell → external network | Always-on `exfil_guard` (secret/credential/upload shapes) | Arbitrary outbound once a command evades the heuristics — the sandbox has open egress; `exfil_guard` is the exfiltration boundary |
