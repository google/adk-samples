...
- **Smallest embeddable harness** — `examples/minimal_agent.py` wired by hand.
...
[`examples/minimal_agent.py`](../examples/minimal_agent.py) sets the lowest-friction
env (Gemini, host tools, in-memory everything, core routes only) and serves the
app:
