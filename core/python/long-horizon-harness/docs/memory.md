...
- **Deploy:** `VertexAiMemoryBankService` (an Agent Runtime with
  Memory Bank enabled).
...
1. **Consolidates a native Structured Profile** — schema in
   ...
   applied to the Agent Runtime by `scripts/provision_agent_engine.py`. The
...
| Structured Profile schema | applied to Agent Runtime at provision time | `horizon/infrastructure/memory_config.py`, `scripts/provision_agent_engine.py` |
