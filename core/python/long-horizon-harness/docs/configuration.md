...
(sessions, state, tasks, memories, uploads, sandbox, secrets, reminders, routines),
...
| Var | Default | Notes |
|---|---|---|
| `LHA_ENVIRONMENT_BACKEND` | `local` | `local` (tools on host) or `sandbox` (Agent Runtime). |
...
| Var | Default | Notes |
|---|---|---|
| `USE_IN_MEMORY_SESSION` | _(off)_ | `true` ⇒ in-process sessions (lost on restart); skips Agent Runtime / Cloud SQL. |
| `SESSION_DB_URL` | — | sqlite/postgres/mysql session store; precedence over Agent Runtime. |
| `AGENT_ENGINE_RESOURCE_NAME` | — | Agent Runtime for sessions **and** Memory Bank. |
