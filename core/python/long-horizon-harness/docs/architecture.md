...
Vertex AI   managed: Gemini, Memory Bank, Agent Runtime sessions, Agent Runtime sandbox
```

Starting from the bottom:

- **Vertex AI** is the managed substrate: the models (Gemini), Memory
  Bank (cross-session memory), and Agent Runtime (resumable session persistence plus the
  per-user BYOC sandbox). Horizon configures these; it does not implement them.
...
cross-session memory"]
    AgentEngine["Agent Runtime sessions
session persistence (resumable)"]
    AgentRuntime["Agent Runtime sandbox
...
1. **Environment interface** — Horizon's `Environment` (`horizon/environment/base.py`, a
   ...
   (`horizon/environment/local.py:LocalEnvironment`) or in a per-user Agent Runtime
   sandbox — pluggable via `LHA_ENVIRONMENT_BACKEND` (string) or
...
2. **Tool guardrails** — the ordered `before_tool` chain (`exfil_guard` Layer A →
   ...
   `halt_reason`). The sandbox runs with open outbound internet egress, so Layer A
   (`exfil_guard`) is the exfiltration boundary (secret material, credential reads,
   metadata-server access, upload-shaped commands to non-allowlisted hosts). See
...
- **Resumability** — `ResumabilityConfig(is_resumable=True)`, one line; durable session
  persistence is Vertex Agent Runtime. The custom value is the delegate *child* driver in
...
- **Resumability** — sessions persist in Agent Runtime and resume; the context cache and
...
- **GCP / Vertex coupling** — the managed-first choice ties deploy to Vertex AI (models,
  Memory Bank, Agent Runtime) and the supporting Cloud Run / Cloud SQL / IAP.
