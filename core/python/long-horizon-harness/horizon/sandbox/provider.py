...
def _sandbox_location() -> str:
    ...
    return os.environ.get("LHA_SANDBOX_LOCATION", SANDBOX_LOCATION)
...
def _try_reattach(client: Any, sandbox_name: str) -> tuple[str, str, str] | None:
    ...
    try:
        ...
        if code == 404:
            ...
            logger.warning("reattach: sandboxes.get(%s) failed (%s)", sandbox_name, exc)
        return None
...
def _template_internet_access(client: Any, template_ref: str, *, default: bool) -> bool:
...
def _sandbox_template(client: Any, sandbox_name: str) -> Any | None:
    ...
    if not template_ref:
        return None
    try:
        return client.agent_engines.sandboxes.templates.get(name=str(template_ref))
    except Exception:
        return None


def _sandbox_internet_access(client: Any, sandbox_name: str, *, default: bool) -> bool:
...
def _reattach_or_none(
    ...
    internet_access = _template_internet_access(client, template_ref, default=True)
...
def _restore_from_snapshot_or_none(
    ...
    try:
        provisioned = restore_sandbox_from_snapshot(
            ...
            ttl=os.environ.get("LHA_SANDBOX_TTL", "").strip() or DEFAULT_SANDBOX_TTL,
        )
    except Exception as exc:
        logger.warning(
    ...
    sandbox_token = mint_sandbox_token(client, service_account=caller_sa)
    minter, fetcher = _build_refresh_closures(
        _vertex_client_factory, caller_sa=caller_sa, sandbox_name=provisioned.name
    )
...
def _provision_fresh_sandbox(
    ...
    *,
    ...
    pending = (
        PendingMigration(client=client, source_env=source_env, source_name=prior_name)
...
class LocalProvider:
    ...
    def build_environment(
        ...
        return (
            LocalEnvironment(working_dir=_default_local_root() / "users" / user_id),
            None,
        )
...
class VertexSandboxProvider:
    ...
    def build_environment(
        ...
        if missing:
            msg = f"LHA_ENVIRONMENT_BACKEND=sandbox requires {', '.join(missing)}"
            if _in_prod():
    ...
    def build_routine_environment(
        ...
        from horizon.sandbox.lifecycle import find_routine_sandbox, routine_display_name
    ...
    def provision_upgrade(self, user_id: str) -> UpgradeProvision:
        ...
        try:
            ...
            logger.warning("provision_upgrade: engine resolution failed: %s", exc)
            return UpgradeProvision(
        ...
        return UpgradeProvision(
            status="provisioned", version=version, new_env=new_env, pending=pending
        )
    ...
    def snapshot_and_prune(self, user_id: str) -> dict[str, Any]:
        ...
        return snapshot_and_prune_user(
            ...
            agent_instance_name=agent_instance,
            user_id=user_id,
            snapshot_ttl=os.environ.get("LHA_SNAPSHOT_TTL", "").strip() or "30d",
            keep=keep,
        )
