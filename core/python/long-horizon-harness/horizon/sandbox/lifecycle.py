...
def ensure_template(
    ...
    *,
    ...
    existing = client.agent_engines.sandboxes.templates.list(name=agent_instance_name)
...
def mint_sandbox_token(client: Any, *, service_account: str) -> str:
    ...
    """
    return str(client.agent_engines.sandboxes.generate_access_token(service_account))
...
def _user_snapshots(
    ...
    *,
    ...
    for snap in client.agent_engines.sandboxes.snapshots.list(name=agent_instance_name):
...
def snapshot_and_prune_user(
    ...
    *,
    ...
    pruned = prune_user_snapshots(
        client, agent_instance_name=agent_instance_name, user_id=user_id, keep=keep
    )
...
def _sanitize_user_id(user_id: str) -> str:
    return _SAFE_USER_ID.sub("_", user_id or "")[:_USER_ID_MAX_LEN] or "anonymous"
...
def find_user_sandbox(
    ...
    *,
    ...
    matches: list[str] = []
    for sandbox in client.agent_engines.sandboxes.list(name=agent_instance_name):
...
def version_below_floor(display_name: str, floor: str) -> bool:
    ...
    try:
        return Version(token.replace("_", ".").lstrip("v")) < Version(floor.lstrip("v"))
