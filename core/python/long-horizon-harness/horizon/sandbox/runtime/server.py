...
except ModuleNotFoundError:  # imported as a package on the host, not from /opt/runtime
...
WORKSPACE_ROOT = Path(os.environ.get("SANDBOX_WORKSPACE", "/workspace")).resolve()
DEFAULT_EXEC_TIMEOUT_SEC = float(os.environ.get("SANDBOX_EXEC_TIMEOUT_SEC", "600"))
...
async def exec_command(req: ExecRequest) -> ExecResponse:
    ...
    timeout = req.timeout if req.timeout is not None else DEFAULT_EXEC_TIMEOUT_SEC
...
@app.post("/files")
async def write_file(req: FileWriteRequest, path: str = Query(...)) -> dict[str, str]:
    target = _resolve_within_workspace(path)
    try:
...
@app.post("/files/zip")
async def upload_zip(request: Request, path: str = Query(...)) -> dict[str, str]:
    target = _resolve_within_workspace(path)
...
def _resolve_env(requested: dict[str, str] | None) -> dict[str, str] | None:
    if requested is None:
        return None
    base = {k: os.environ[k] for k in _PROCESS_INHERITED_ENV_KEYS if k in os.environ}
...
class _ContainerProcess:
    ...
    def read(self, offset: int = 0, limit: int | None = None) -> tuple[bytes, int]:
        with self._lock:
...
async def process_kill(sid: str) -> ProcessKillResponse:
    ...
    return ProcessKillResponse(exit_code=exit_code if exit_code is not None else -1)
