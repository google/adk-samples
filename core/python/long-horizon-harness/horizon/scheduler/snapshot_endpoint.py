...
async def snapshot_tick(payload: SnapshotRequest, request: Request) -> Any:
    ...
    if session_service is None or not app_name:
        return {"snapshotted": 0, "failed": 0, "reason": "runner not registered"}
    ...
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"snapshotted": 0, "failed": len(user_ids), "errors": user_ids},
        )
    ...
    for user_id in user_ids:
        try:
            ...
            logger.exception("snapshot_tick: per-user pass raised for %s", user_id)
            failed += 1
