...
def set_routine_secret_scope(names: Iterable[str]) -> contextvars.Token:
    ...
    return _routine_secret_scope.set(frozenset(n for n in names if isinstance(n, str)))
...
_delegated_google_token: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "delegated_google_token", default=None
)
