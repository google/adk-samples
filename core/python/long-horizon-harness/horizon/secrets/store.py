...
class SecretManagerStore:
    ...
    async def _fetch_blob(self, user_id: str) -> dict[str, dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("secret blob for user is not valid JSON; treating as empty")
            return {}
    ...
    async def _write_blob(self, user_id: str, blob: dict[str, dict[str, Any]]) -> None:
...
class InMemorySecretStore:
    ...
    def _guard_size(self, blob: dict[str, dict[str, Any]]) -> None:
        if len(json.dumps(blob).encode("utf-8")) > MAX_BLOB_BYTES:
            raise SecretTooLargeError(f"secrets blob is over {MAX_BLOB_BYTES} bytes")
