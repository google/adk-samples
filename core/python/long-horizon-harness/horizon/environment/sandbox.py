...
class SandboxEnvironment(Environment):
    ...
    async def initialize(self) -> None:
        ...
        await self._wait_for("/healthz", _HEALTHZ_TIMEOUT_SEC, self._probe_healthz)
    ...
    async def refresh_auth(self) -> bool:
        ...
        try:
            ...
            logger.warning("sandbox JWT refresh failed (%s); using cached token", exc)
            return True
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            logger.warning("SandboxEnvironment.close: http aclose failed: %s", exc)
    ...
    async def read_file(self, path: Path) -> bytes:
        r = await self._http.get("/files", params={"path": str(path)}, timeout=60.0)
        if r.status_code == 200:
            try:
                envelope = r.json()
    ...
    def _raise_file_http_error(self, op: str, path: Path, r: httpx.Response) -> None:
        detail = r.text
        try:
