...
def resolve_registry(tool_context: Any | None) -> ProcessRegistry:
    key = session_key_from_context(tool_context) if tool_context is not None else None
    if key is not None:
    ...
    logger.debug("processes: no session key; falling back to temp: state registry")
...
class ProcessRegistry:
    ...
    def _prune_session_registries() -> None:
        ...
        while len(_SESSION_REGISTRIES) > DEFAULT_MAX_SESSION_REGISTRIES:
            protected = next(reversed(_SESSION_REGISTRIES))  # most-recently moved
            evicted = False
