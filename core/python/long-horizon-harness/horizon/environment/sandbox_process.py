...
class SandboxProcessHandle:
    ...
    def idle_seconds(self) -> float:
        ...
        return self._last_idle_seconds + max(0.0, time.time() - self._last_status_at)
    ...
    async def read(
        ...
        if limit is not None:
            params["limit"] = limit
        r = await self._http.get(f"/processes/{self.session_id}/output", params=params)
        r.raise_for_status()
        body = r.json()
    ...
    async def write(self, data: bytes) -> None:
        ...
        r = await self._http.post(f"/processes/{self.session_id}/stdin", json=payload)
