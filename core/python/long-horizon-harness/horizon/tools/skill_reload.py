...
def host_mirror_dir(working_dir: Path, owner: str | None = None) -> Path:
    ...
    """
    digest = hashlib.sha256(f"{owner or ''}\0{working_dir}".encode()).hexdigest()[:16]
...
async def bind_session_skills_callback(callback_context: Any) -> None:
    ...
    mirror_root = host_mirror_dir(Path(env.working_dir), getattr(env, "owner", None))
    builtin_dir = builtin_skills_root()
    try:
        cached = _mirror_cache.get(str(mirror_root))
        if cached is None:
            user_dir = await mirror_user_skills_to_host(env, dest_root=mirror_root)
            ...
            _mirror_cache[str(mirror_root)] = (user_dir, _TOOLSET._skills, catalog)
        else:
