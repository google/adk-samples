...
def walk_skill_dirs(*, user_dir: Path, builtin_dir: Path) -> dict[str, models.Skill]:
...
def build_skill_toolset(
    *, user_dir: Path, builtin_dir: Path | None = None
    ...
    """
    resolved_builtin = builtin_dir if builtin_dir is not None else builtin_skills_root()
    skills_dict = walk_skill_dirs(user_dir=user_dir, builtin_dir=resolved_builtin)
