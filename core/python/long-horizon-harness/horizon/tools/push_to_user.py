...
def _build_payload(*, channel: str, recipient_id: str, text: str) -> dict[str, Any]:
    return {
        "jsonrpc": "2.0",
        "id": uuid.uuid4().hex,
        "method": "message/send",
...
async def push_to_user(
    *,
    ...
    payload = _build_payload(channel=channel, recipient_id=recipient_id, text=text)
    ...
    try:
        async with httpx.AsyncClient(timeout=_REQUEST_TIMEOUT_S) as client:
            response = await client.post(push_url, json=payload, headers=headers)
    except Exception as exc:
