...
async def _list_entries(session_id: str | None = None) -> list[dict[str, str]]:
    ...
    result = await active_environment().execute(f"python3 -c {shlex.quote(script)}")
    ...
    try:
        ...
        return []
    return [e for e in entries if isinstance(e, dict) and "name" in e and "text" in e]
...
async def read_todos(session_id: str | None = None) -> list[dict[str, Any]]:
    ...
    return [parse_todo(entry["text"]) for entry in await _list_entries(session_id)]
...
async def reconcile_todos(
    ...
    for entry in await _list_entries(session_id):
        if entry["name"] not in keep:
            await env.execute(f"rm -f {shlex.quote(str(folder / entry['name']))}")
