...
def workspace_download_url(workspace_rel_path: str, *, inline: bool = True) -> str:
...
def _signed_blob_url(
    ...
    try:
        ...
        logger.warning(
            "signed-url mint failed for gs://%s/%s", bucket, blob_name, exc_info=True
        )
        return None
...
def artifact_url(
    *,
    ...
    """
    bucket = os.environ.get("LOGS_BUCKET_NAME", "").strip().removeprefix("gs://")
    if bucket:
