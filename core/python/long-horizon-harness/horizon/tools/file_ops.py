...
from horizon.workspace_window import maybe_seed_window, resolve_in_window, window_dirs
...
def _resolve_in_session_window(
    ...
    return resolve_in_window(path, env_root, window_dirs(_state_of(tool_context)))
...
def _format_numbered_lines(
    ...
    for i, line in enumerate(window):
        if len(line) > MAX_LINE_LENGTH:
            line = line[:MAX_LINE_LENGTH] + _LINE_TRUNC_SUFFIX
...
async def _post_edit_diagnostics(path: str) -> str | None:
    ...
    command = f"ruff check --output-format=json --force-exclude {shlex.quote(path)}"
    try:
...
async def search_files(
    ...
    *,
    ...
    env_root = active_environment().working_dir.resolve()
    window = [] if scope == "workspace" else window_dirs(_state_of(tool_context))
