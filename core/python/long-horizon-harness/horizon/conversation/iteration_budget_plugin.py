...
_BUDGET_EXCEPTIONS = (IterationLimitExceeded, ToolCallLimitExceeded, AgentHalted)
...
class IterationBudgetPlugin(BasePlugin):
    ...
    def _hydrate_budget(self, state: dict[str, Any]) -> IterationBudget:
        ...
        budget._tool_calls_this_iteration = int(state.get(_TOOL_CALLS_STATE_KEY, 0))
        if state.get(_HALTED_STATE_KEY):
            budget._halted = True
            budget._halt_reason = state.get(_HALT_REASON_STATE_KEY)
        return budget

    def _persist_budget(self, state: dict[str, Any], budget: IterationBudget) -> None:
