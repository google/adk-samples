...
def discover_context_files(cwd: str | Path) -> list[tuple[str, str]]:
    ...
    for name in CONTEXT_FILENAMES:
        ...
        if len(encoded) > MAX_CONTEXT_FILE_BYTES:
            text = (
                encoded[:MAX_CONTEXT_FILE_BYTES].decode("utf-8", errors="ignore")
                + "\n\n[truncated]"
            )
        return [(name, text)]
    return []
...
def _format_cli_versions(versions: dict[str, str | None]) -> str | None:
    lines = [f"- {name}: {version}" for name, version in versions.items() if version]
    if not lines:
        return None
...
def _is_sandbox_backend() -> bool:
    return os.environ.get("LHA_ENVIRONMENT_BACKEND", "").strip().lower() == "sandbox"
...
def _ensure_stable_tier(callback_context, llm_request: LlmRequest, cwd: str) -> None:
    config = llm_request.config
    existing = getattr(config, "system_instruction", None)
    has_caller_instruction = isinstance(existing, str) and bool(existing.strip())
...
async def _available_secrets_line(user_id: str | None) -> str:
    ...
    try:
        ...
        names = [s["name"] for s in await get_secret_store().list_names(user_id)]
    except Exception:
