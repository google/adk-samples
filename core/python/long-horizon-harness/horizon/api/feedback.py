...
async def _feedback_context_summary(runner: Any, user_id: str, session_id: str) -> str:
    if runner is None:
        return ""
    try:
        ...
        logger.exception("feedback: get_session failed for %s/%s", user_id, session_id)
        return ""
...
def attach_feedback_routes(app: FastAPI, *, runner: Any) -> None:
    ...
    async def collect_feedback(
        ...
        try:
            ...
            logger.exception(
                "feedback: emit raised despite resilient sink; record=%s", record
            )
