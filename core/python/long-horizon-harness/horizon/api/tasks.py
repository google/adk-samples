...
def _state_str(state: int) -> str:
    ...
    return TaskState.Name(state).removeprefix("TASK_STATE_").lower().replace("_", "-")


def attach_task_routes(app: FastAPI, *, runner: Runner, task_store: TaskStore) -> None:
    ...
    async def list_tasks(
        ...
        for task_id in task_ids:
            try:
                ...
                logger.exception("lha_tasks: task_store.get failed for %s", task_id)
                continue
            if task is None:
                continue
            out.append(
