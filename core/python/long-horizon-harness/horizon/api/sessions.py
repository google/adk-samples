...
async def _resolve_title_and_created(
    *, runner: Runner, user_id: str, recent: Any
    ...
    try:
        ...
        logger.exception(
            "lha_sessions: oldest-events fetch failed for %s/%s", user_id, recent.id
        )
        full = None
...
def attach_session_routes(app: FastAPI, *, runner: Runner) -> None:
    ...
    async def list_chats(
        ...
        try:
            ...
            logger.exception("lha_sessions: list_sessions failed for %s", user_id)
            raise HTTPException(status_code=500, detail="list sessions failed") from err
        ...
        ordered = sorted(resp.sessions, key=lambda s: s.last_update_time, reverse=True)
        scan_limit = SESSIONS_PAGE_SIZE if source is None else SCHEDULER_SCAN_LIMIT
        ...
        for stub in scanned:
            try:
                full = await runner.session_service.get_session(
                    ...
                    user_id=user_id,
                    session_id=stub.id,
                    config=GetSessionConfig(num_recent_events=SESSIONS_EVENT_FETCH),
                )
            except Exception:
                logger.exception(
                    "lha_sessions: get_session failed for %s/%s", user_id, stub.id
                )
                continue
            if full is None:
                continue
            ...
            out.append(
                {
                    ...
                    "workspaceWindow": window_dirs(getattr(full, "state", None)),
                }
            )
        ...
        try:
            await runner.session_service.append_event(
                ...
                Event(
                    invocation_id=new_invocation_context_id(),
                    author="system",
                    actions=EventActions(state_delta={TITLE_KEY: body.title.strip()}),
                ),
            )
        except Exception as err:
            logger.exception("lha_sessions: append_event failed for rename")
            raise HTTPException(status_code=500, detail="rename failed") from err
    ...
    async def set_window(
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail="set window failed") from err
    ...
    async def delete_chat(
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail="delete failed") from err
