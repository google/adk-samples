...
def _sanitize_relpath(raw: str) -> PurePosixPath:
    ...
    if any(ord(c) < 32 for c in raw):
        raise HTTPException(
            status_code=400, detail="relpath must not contain control characters"
        )
    ...
    for part in parts:
        ...
        if "/" in part or "\\" in part:
            raise HTTPException(
                status_code=400, detail="relpath segment must not contain separators"
            )
...
def _sanitize_filename(raw: str) -> str:
    ...
    if any(ord(c) < 32 for c in raw):
        raise HTTPException(
            status_code=400, detail="filename must not contain control characters"
        )
    if raw in (".", ".."):
        raise HTTPException(status_code=400, detail="filename must not be '.' or '..'")
    if raw.startswith("."):
        raise HTTPException(status_code=400, detail="filename must not start with '.'")
...
def _resolve_workspace_path(env: Environment, requested: str) -> Path:
    ...
    root = (
        env.working_dir.resolve() if env.working_dir.is_absolute() else env.working_dir
    )
    try:
...
async def _delete_path(env: Environment, target: Path, *, recursive: bool) -> None:
...
async def _serve_dir_zip(env: Environment, target: Path) -> Response:
    try:
        ...
        raise HTTPException(status_code=404, detail=f"{target} not found") from err
...
def attach_uploads_routes(app: FastAPI) -> None:
    ...
    async def workspace_tree(
        ...
        try:
            ...
            raise HTTPException(status_code=404, detail=f"{path} not found") from err
    ...
    async def workspace_download(
        ...
        if inline_mime is not None:
            return Response(
                ...
                headers={
                    "Content-Disposition": _disposition(target.name, inline=True),
    ...
    async def workspace_delete(
        ...
        if target.resolve() == root:
            raise HTTPException(
                status_code=400, detail=f"refusing to delete workspace root {path}"
            )
