...
def grant_matches_loose(grant: dict[str, Any], tool_name: str, args: Any) -> bool:
    ...
    *,
    ...
    if action == "grant":
        if tool_name is None:
            return {"granted": False, "error": "action='grant' requires 'tool_name'"}
        if signature is None:
            return {"granted": False, "error": "action='grant' requires 'signature'"}
        return _grant(tool_name, signature, tool_context)
    if action == "list":
        return _list(tool_context)
    if action == "revoke":
        if index is None:
            return {"revoked": False, "error": "action='revoke' requires 'index'"}
