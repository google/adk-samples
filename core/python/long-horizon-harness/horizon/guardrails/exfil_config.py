...
# here.
GIT_EGRESS_RE = re.compile(r"\bgit\b[^\n]*\b(?:push|remote\s+(?:add|set-url))\b")
...
def default_config() -> ExfilConfig:
    return ExfilConfig(
        ...
        sensitive_path_res=(re.compile(_DEFAULT_SENSITIVE_PATH, re.IGNORECASE),),
    )
...
def _parse_overlay(text: str) -> tuple[list[str], list[str], list[str]]:
    ...
    for obj in iter_jsonl_objects(text, context="exfil"):
        ...
        secrets += [s for s in obj.get("secret_patterns", []) if isinstance(s, str)]
        paths += [p for p in obj.get("sensitive_paths", []) if isinstance(p, str)]
