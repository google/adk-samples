...
def parse_rule(obj: Any, *, trusted: bool = False) -> PermissionRule | None:
    ...
    if not tool_names:
        return None
    command_regex = (
        obj.get("commandRegex") if isinstance(obj.get("commandRegex"), str) else None
    )
    if command_regex is not None and not trusted and not safe_regex(command_regex):
        _logger.warning("permissions: dropping rule with unsafe commandRegex")
        return None
    args_pattern = (
        obj.get("argsPattern") if isinstance(obj.get("argsPattern"), str) else None
    )
    if args_pattern is not None and not trusted and not safe_regex(args_pattern):
    ...
    return PermissionRule(
        ...
        subagent=obj.get("subagent") if isinstance(obj.get("subagent"), str) else None,
...
def resolve_decision(
    ...
    *,
    ...
    matches = [
        ...
        for r in rules
        if rule_matches(
            r, tool_name=tool_name, args=args, command=command, agent_name=agent_name
        )
    ]
...
async def _load_interface_rules(env: Any, working_dir: Path) -> list[PermissionRule]:
    ...
    if cached is not None:
        return list(cached)
    text = await read_overlay_text(env, working_dir / PERMISSION_OVERLAY_FILENAME)
    rules = _parse_jsonl(text)
...
_BENIGN_SIDE_EFFECT_TOOLS: tuple[str, ...] = ("add_memory", "reminder", "reload")
