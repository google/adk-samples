...
_PATH_ARG_NAMES: frozenset[str] = frozenset({"path", "file_path", "target_path"})
...
def _filter_unsafe_regexes(obj: dict[str, Any]) -> dict[str, Any]:
    ...
    if filtered:
        ...
        obj = {k: v for k, v in obj.items() if k != "destructive_commands_regex"}
    return obj
...
def _evaluate(rule: dict[str, Any], tool_name: str, args: Any) -> str | None:
        ...
        for arg_name, patterns in destructive_commands_regex.items():
            ...
            for pat in patterns:
                ...
                try:
                    ...
                    _logger.warning("policies: invalid regex %r in rule, skipping", pat)
    ...
    if isinstance(destructive_path_patterns, list) and isinstance(args, dict):
        globs = [p for p in destructive_path_patterns if isinstance(p, str) and p]
        for arg_name, value in args.items():
            if arg_name not in _PATH_ARG_NAMES:
                continue
            if not isinstance(value, str):
            ...
            for pat in globs:
                if fnmatch.fnmatchcase(value, pat):
                    return (
                        f"destructive path pattern {pat!r} matched on arg {arg_name!r}"
                    )
    ...
    if command is not None:
        ...
        if verdict is not None:
            ...
            if tier == "deny" or ask_is_deny:
                return _block(why, {"command_safety": tier}, source="command_safety")
            return None
    return None
