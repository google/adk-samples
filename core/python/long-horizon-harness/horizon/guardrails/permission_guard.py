...
SELF_CONFIRMING_TOOLS: frozenset[str] = frozenset({"clarify", "report_to_maintainers"})
...
def _shell_decision(
    rules: list[PermissionRule], tool_name: str, command: str, agent_name: str | None
...
async def resolve_permission_decision(
    ...
    *,
    ...
    decision, matched = resolve_decision(
        rules, tool_name=tool_name, args=args, command=None, agent_name=agent_name
    )
...
def _resolve_outcome(payload: Any, ordered_outcomes: list[tuple[str, str]]) -> str:
...
async def permission_guard(
    *, tool: Any, args: Any, tool_context: Any
    ...
    summary = f"Run: {command}" if command is not None else f"Allow tool: {tool_name}"
    ...
    if outcome == "proceed_always":
        write_session_grants(state, [*read_session_grants(state), proposed_rule])
