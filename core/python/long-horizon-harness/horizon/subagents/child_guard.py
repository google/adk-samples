...
from horizon.subagents.resurface_context import bubble_budget, try_consume_bubble
...
def _write_grant(state: Any, tool_name: str, proposed_prefix: list[str] | None) -> None:
    if state is None:
        return
...
async def _resurface_decision(
    *, tool_name: str, args: Any, tool_context: Any, state: Any
    ...
    decision, deny_rule, proposed_prefix, command = await resolve_permission_decision(
        _active_env(),
        tool_name=tool_name,
        args=args,
        state=state,
    ...
    *,
    ...
    async def guard(
        *, tool: Any, args: Any, tool_context: Any
        ...
        if grants and state is not None and not state.get(POLICY_GRANTS_STATE_KEY):
            try:
                state[POLICY_GRANTS_STATE_KEY] = list(grants)
            except (AttributeError, TypeError):
                pass

        exfil = await exfil_guard(tool=tool, args=args, tool_context=tool_context)
        ...
        if bubble_budget() is None:
            ...
            return await policies_guard(
                tool=tool, args=args, tool_context=tool_context, ask_is_deny=True
            )
        ...
        return await _resurface_decision(
            tool_name=tool_name, args=args, tool_context=tool_context, state=state
        )
