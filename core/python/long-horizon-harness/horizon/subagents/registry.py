...
class SubAgentHandle:
    ...
    name: str | None = None
    consumed: bool = False  # returned by wait(); don't surface it again next wait()
...
class SubAgentRegistry:
        ...
        *,
        ...
        async with self._lock:
            if task_ids is None:
                ...
                handles = [self._handles[t] for t in task_ids if t in self._handles]
        if not handles:
    ...
    async def cancel(self, task_id: str) -> bool:
        async with self._lock:
            handle = self._handles.get(task_id)
            children = [h for h in self._handles.values() if h.parent_id == task_id]
        if handle is None:
            return False
