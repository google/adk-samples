...
async def build_child_agent(
    *,
    ...
    if resolved_profile is not None and resolved_profile.allowed_tool_names is not None:
        allowed = resolved_profile.allowed_tool_names
        resolved_tools = [t for t in resolved_tools if _callable_name(t) in allowed]
    ...
    agent_kwargs["before_tool_callback"] = [
        make_child_policy_guard(parent_grants=parent_grants, profile=resolved_profile)
    ]
