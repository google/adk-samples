...
def _halt(summary: str, *, telemetry: dict[str, Any] | None = None) -> dict[str, Any]:
    return {
        "status": "halted",
        "summary": summary,
...
async def _delegate_resurfacing(
    *,
    ...
    if confirmation is None:
        ...
        with child_drain(1):
            res = await drive_child(
                ...
                session_id=child_session_id,
                new_message=Content(
                    role=_KICKOFF_CONTENT_ROLE, parts=[Part(text=_KICKOFF_MESSAGE)]
                ),
                timeout_s=timeout_s,
                max_iterations=max_iterations,
            )
        if res.pending is not None:
            parent_state[f"{_CHILD_CONF_KEY}{fc_id}"] = res.pending.confirmation_id
            hint = _bubble_hint(name, res.pending.hint)
            tool_context.request_confirmation(hint=hint, payload=res.pending.payload)
            return {
    ...
    if not conf_id:
        await _cleanup_child_session(session_service, user_id, child_session_id)
        return _halt("Lost the child's pending-approval state; re-run the task.")
