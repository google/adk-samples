...
def _sync_row(tool_context: ToolContext | None, task_id: str, payload: dict) -> None:
...
async def _spawn(
    *,
    ...
    task_id = await registry.register(coro_factory=_factory, goal=goal, name=name)
...
async def _status(task_id: str, tool_context: ToolContext | None) -> dict[str, Any]:
...
async def _result(
    ...
    payload = await get_registry().get_result(task_id, wait=wait, timeout_s=timeout_s)
    _sync_row(tool_context, task_id, payload)
    return payload


async def _wait(
    task_ids: list[str] | None, timeout_s: float, tool_context: ToolContext | None
) -> dict[str, Any]:
    ...
    if tid is not None:
        _sync_row(
            tool_context,
            tid,
            {"status": payload["status"], "result": payload.get("result") or {}},
        )
    return payload
...
async def agent(
    ...
    *,
    ...
    if action == "status":
        if task_id is None:
            return {"success": False, "error": "action='status' requires 'task_id'"}
        return await _status(task_id, tool_context)
    if action == "result":
        if task_id is None:
            return {"success": False, "error": "action='result' requires 'task_id'"}
