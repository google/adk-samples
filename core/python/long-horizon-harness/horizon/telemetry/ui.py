...
def _is_ok(response: Any) -> bool:
    if isinstance(response, dict):
        ...
        if isinstance(status, str) and status.lower() in {"error", "failed", "halted"}:
            return False
...
def _append_capped(state: Any, key: str, entry: dict[str, Any], cap: int) -> None:
...
def record_memory_write(
    *,
    ...
    try:
        ...
        _append_capped(tool_context.state, MEMORY_WRITES_STATE_KEY, entry, _MEMORY_CAP)
    except Exception:
        pass
