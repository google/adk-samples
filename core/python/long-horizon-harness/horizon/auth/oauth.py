...
WORKSPACE_SURFACES: dict[str, dict[str, tuple[str, ...]]] = {
    ...
    "meet": {"ro": ("meetings.space.readonly",), "rw": ("meetings.space.created",)},
...
GWS_META_KEY = (
    "GOOGLE_WORKSPACE_SCOPES_META"  # json: {"surfaces": [...], "readonly": bool}
)
...
async def persist_gcp(store: Any, user_id: str, *, token: str, expires_at: int) -> None:
...
def attach_gcp_oauth_routes(app: FastAPI) -> None:
    ...
    async def connect(
        ...
        if target == "gcp":
            ...
            if not scopes:
                raise HTTPException(
                    status_code=400, detail="select at least one Workspace surface"
                )
        else:
            raise HTTPException(
    ...
    async def callback(
        ...
        if data.get("target") == "workspace":
            ...
            await persist_gcp(store, user_id, token=token, expires_at=expires_at)
