...
def _resolve(state: dict[str, Any] | None) -> str:
    if state:
        ...
        if choice in MODEL_REGISTRY:
            return choice
        if choice:
            logger.warning("selected_model=%r not in registry; falling back", choice)

    env_choice = os.environ.get("LHA_ROOT_MODEL")
    if env_choice in MODEL_REGISTRY:
        return env_choice
    if env_choice:
        logger.warning("LHA_ROOT_MODEL=%r not in registry; falling back", env_choice)
...
async def select_model_callback(*, callback_context: Any, llm_request: Any) -> None:
    state = getattr(callback_context, "state", None)
    model_name = _resolve(state)
    llm_request.model = model_name
    apply_compaction_threshold(_compaction_config_from(callback_context), model_name)
    return None
