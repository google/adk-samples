...
from horizon.guardrails.policy_grants import POLICY_GRANTS_STATE_KEY, policy_grant
...
async def _sandbox_upgrade(args: str, callback_context: Any) -> str:
    ...
    return (
        f"/sandbox-upgrade unavailable: {result.get('reason', 'sandbox backend off')}"
    )
...
async def _permissions(args: str, callback_context: Any) -> str:
    ...
    lines.append(f"{len(persisted)} persisted rule(s) in .lha/permissions.jsonl")
...
async def _routines(args: str, callback_context: Any) -> str:
    ...
    if sub == "remove" and len(parts) > 1:
        ...
        return f"Removed routine '{parts[1].strip()}'." if ok else "No such routine."

    rows = await store.list_for_user(user_id)
    if not rows:
        return "No routines scheduled."
    lines = [
        f"- {r.id}  ({r.schedule})  next: {r.next_fire_at.isoformat()}" for r in rows
    ]
