...
from horizon.infrastructure.memory_config import USER_PROFILE_SCHEMA_ID, profile_scope
...
_ACTION_KEYS = {"CREATED": "created", "UPDATED": "updated", "DELETED": "deleted"}
...
class MemoryAdapter(Protocol):
    ...
    async def generate_profile(
        self, *, app_name: str, user_id: str, events: list[Any], consolidate: bool
    ) -> dict[str, int]: ...
    async def list_all(
        self, *, app_name: str, user_id: str
...
class NoopMemoryAdapter:
    ...
    async def generate_profile(
        ...
        raise NotImplementedError("this memory backend has no structured profiles")

    async def list_all(self, *, app_name: str, user_id: str) -> list[MemoryWriteEntry]:
        return []
...
class VertexMemoryAdapter:
    ...
    async def retrieve_profile(
        ...
        try:
            response = await self._client().agent_engines.memories.retrieve_profiles(
