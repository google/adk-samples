...
async def _collect_user_sessions(
    ...
    sessions.sort(key=lambda s: getattr(s, "last_update_time", 0) or 0, reverse=True)
...
async def _run_dream_review_for_user(
    *,
    ...
    events = [
        event for session in sessions for event in session["events"] if _has_text(event)
    ]
    if not events:
        return {"success": False, "reason": "no recent session content to review"}
