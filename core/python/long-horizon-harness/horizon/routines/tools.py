...
async def _create(
    *, name: str, schedule: str, task: str, secrets, delivery, tool_context
    ...
    if not (name and schedule and task):
        return {"success": False, "error": "create requires name, schedule, task"}
    ...
    if not getattr(tc, "confirmed", False):
        return {"success": False, "status": "declined", "error": "user declined"}

    # confirmed → backend writes the manifest + registers the schedule row
    await write_routine_via_env(active_environment(), body, routine_id=routine_id)
...
async def _test(*, name: str, task: str, secrets, tool_context) -> dict[str, Any]:
    if not task:
