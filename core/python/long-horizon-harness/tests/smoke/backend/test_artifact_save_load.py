...
async def test_save_then_load_round_trips_payload_and_filename(
    ...
    assert tool_context.actions.artifact_delta.get(workspace_path.name) is not None
...
async def test_save_missing_file_returns_error_not_exception(
    ...
    result = await artifact(action="save", path=missing, tool_context=tool_context)
