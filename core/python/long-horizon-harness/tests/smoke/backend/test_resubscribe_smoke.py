...
class _NoopExecutor:
    async def execute(self, *_a, **_k):  # pragma: no cover - resubscribe never executes
        return None
...
async def smoke_client() -> TestClient:
    ...
    # sandbox).
    app = FastAPI()
    agent = Agent(name="smoke_agent", model="gemini-flash-latest", description="smoke")
...
def test_resubscribe_unknown_task_carries_error(smoke_client: TestClient) -> None:
