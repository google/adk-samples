...
def _send_and_time_first_byte(base_url: str, *, user_id: str, text: str) -> float:
    ...
    for raw in response.iter_lines():
        ...
        if result.get("final") is True or state in {"completed", "failed", "canceled"}:
...
def test_second_request_for_same_user_is_warm(smoke_http_server: str) -> None:
    ...
    cold = _send_and_time_first_byte(smoke_http_server, user_id=user_id, text=prompt)
    warm = _send_and_time_first_byte(smoke_http_server, user_id=user_id, text=prompt)
