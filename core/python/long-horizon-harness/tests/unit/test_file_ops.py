...
class TestReadFile:
    ...
    async def test_offset_starts_at_requested_line(self, tmp_path: Path) -> None:
...
class TestReadFileNumbering:
    ...
    async def test_long_line_is_capped_with_suffix(self, tmp_path: Path) -> None:
    ...
    async def test_continuation_trailer_respects_offset(self, tmp_path: Path) -> None:
        from horizon.tools.file_ops import read_file
...
class TestWriteFile:
    ...
    async def test_deny_list_blocks_ssh_authorized_keys(
        ...
        assert result["success"] is False
        assert (
            "denied" in result["error"].lower() or "refused" in result["error"].lower()
        )
    ...
    async def test_deny_list_blocks_aws_credentials(
        ...
        result = await write_file(str(target), "[default]\naws_access_key_id=...")
...
class TestPatch:
    ...
    async def test_missing_old_string_fails_and_leaves_file(
        ...
        result = await patch(str(target), "this text is nowhere in the file", "x")
    ...
    async def test_replace_all_swaps_every_occurrence(self, tmp_path: Path) -> None:
        from horizon.tools.file_ops import patch
...
class TestSearchFiles:
    ...
    async def test_finds_match_in_nested_directory(self, tmp_path: Path) -> None:
        from horizon.tools.file_ops import search_files
    ...
    async def test_file_glob_filters_extension(self, tmp_path: Path) -> None:
        ...
        result = await search_files("needle", path=str(tmp_path), file_glob="*.py")
    ...
    async def test_missing_path_returns_error(self, tmp_path: Path) -> None:
        ...
        result = await search_files("anything", path=str(tmp_path / "nonexistent_dir"))
...
class TestSearchFilesRegex:
    ...
    async def test_invalid_regex_returns_error_not_crash(self, tmp_path: Path) -> None:
        from horizon.tools.file_ops import search_files
    ...
    async def test_matches_include_mtime(self, tmp_path: Path) -> None:
        ...
        assert result["success"] is True
        assert result["matches"]
        assert all(isinstance(m["mtime"], (int, float)) for m in result["matches"])

    async def test_results_sorted_by_mtime_descending(self, tmp_path: Path) -> None:
...
class TestModuleSurface:
    ...
    def test_exports_four_tools(self) -> None:
        ...
        for name in ("read_file", "write_file", "patch", "search_files"):
            assert hasattr(file_ops, name), f"horizon.tools.file_ops must export {name}"
            assert callable(getattr(file_ops, name))


class TestRoundTrips:
    async def test_write_then_read_returns_same_content(self, tmp_path: Path) -> None:
