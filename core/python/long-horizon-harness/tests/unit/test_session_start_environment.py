...
def _fake_context(
    ...
    return SimpleNamespace(user_id=user_id, state=state if state is not None else {})
...
def _fake_vertex_client(
    *,
        ...
        *, name: str, display_name: str, config: Any = None
        ...
        pool.append(
            SimpleNamespace(
                ...
                egress_control_config=SimpleNamespace(internet_access=(mode == "net")),
    ...
    def fake_engine_create(*, config: Any = None) -> Any:
        ...
        new_name = (
            f"projects/p/locations/{SANDBOX_LOCATION}/reasoningEngines/r-{len(engines)}"
        )
        engine = SimpleNamespace(
            api_resource=SimpleNamespace(display_name=display_name, name=new_name)
        )
    ...
    snapshot_mgr = SimpleNamespace(list=fake_snapshot_list, delete=fake_snapshot_delete)
    ...
    client.agent_engines.sandboxes.generate_access_token = fake_generate_access_token
    return client, calls
...
async def test_sandbox_backend_with_agent_instance(
    ...
    _set_sandbox_env(monkeypatch, tmp_path=tmp_path, agent_instance=agent_instance)
    ...
    monkeypatch.setattr(SandboxEnvironment, "initialize", _async_noop, raising=True)
...
async def test_sandbox_backend_reattaches_to_discovered_sandbox(
    ...
    existing_sandbox = (
        "projects/p/locations/l/reasoningEngines/r/sandboxEnvironments/sb-alice-prior"
    )
...
async def test_sandbox_backend_reprovisions_when_discovered_sandbox_gone(
    ...
    gone = "projects/p/locations/l/reasoningEngines/r/sandboxEnvironments/sb-gone"
    ...
    client, calls = _fake_vertex_client(
        existing_sandboxes=[_sandbox_stub(name=gone, display_name=_dn("alice"))],
...
async def test_sandbox_backend_reprovisions_when_discovered_sandbox_not_running(
    ...
    stale = "projects/p/locations/l/reasoningEngines/r/sandboxEnvironments/sb-stale"
    client, calls = _fake_vertex_client(
        existing_sandboxes=[
            _sandbox_stub(
...
def _running_sandbox(
    name: str, *, template_mode: str | None = None, template_ref: str | None = None
) -> SimpleNamespace:
    ref = (
        template_ref
...
async def test_scheduler_session_reattaches_version_agnostically(
    ...
    from horizon.environment.sandbox import SandboxEnvironment
    from horizon.environment_context import active_environment
    from horizon.infrastructure.constants import SCHEDULER_SOURCE, SESSION_SOURCE_KEY
    ...
    await on_session_start_callback(
        _fake_context(user_id="alice", state={SESSION_SOURCE_KEY: SCHEDULER_SOURCE})
    )
...
async def test_subfloor_sandbox_force_upgrades_and_migrates(
    ...
    monkeypatch.setattr(SandboxEnvironment, "download_zip", fake_download, raising=True)
    monkeypatch.setattr(SandboxEnvironment, "upload_zip", fake_upload, raising=True)
...
async def test_fresh_sandbox_is_hermetic_by_default_in_prod(
    ...
    assert calls["template_create"][0]["display_name"] == template_display_name(False)
...
async def test_fresh_sandbox_has_internet_by_default_in_dev(
    ...
    assert calls["template_create"][0]["display_name"] == template_display_name(True)
...
async def test_reattach_recovers_internet_access_from_template(
    ...
    from horizon.conversation.session_start import _ensure_environment, _env_cache
    from horizon.environment.sandbox import SandboxEnvironment
    ...
    for mode, expected in (("net", True), ("hermetic", False), (None, True)):
        ...
        client, _ = _fake_vertex_client(
            existing_sandboxes=[_sandbox_stub(name=prior, display_name=_dn("alice"))],
            sandbox_get_response=_running_sandbox(prior, template_mode=mode),
        )
        monkeypatch.setattr(provider, "_vertex_client_factory", lambda c=client: c)
        env = await _ensure_environment("alice")
        assert env.internet_access is expected, mode


def _build_routine_env(monkeypatch: pytest.MonkeyPatch, agent_instance: str) -> Any:
...
async def test_upgrade_user_sandbox_preserves_internet_mode(
    ...
    agent_instance = "projects/p/locations/l/reasoningEngines/r"
    _set_sandbox_env(monkeypatch, tmp_path=tmp_path, agent_instance=agent_instance)
    monkeypatch.delenv("LHA_SANDBOX_INTERNET_ACCESS", raising=False)  # fleet = deny
...
async def test_sandbox_backend_auto_discovers_engine_by_display_name(
    ...
    existing = f"projects/p/locations/{SANDBOX_LOCATION}/reasoningEngines/r-existing"
    client, calls = _fake_vertex_client(
...
async def test_sandbox_backend_creates_engine_when_not_found(
    ...
    assert getattr(created_config, "display_name", None) == DEFAULT_ENGINE_DISPLAY_NAME
...
async def test_sandbox_backend_without_runtime_image_falls_back_to_local(
    ...
    monkeypatch.setenv("LHA_ENVIRONMENT_BACKEND", "sandbox")
    monkeypatch.setenv(
        "AGENT_ENGINE_RESOURCE_NAME", "projects/p/locations/l/reasoningEngines/r"
    )
    monkeypatch.delenv("LHA_RUNTIME_IMAGE", raising=False)
    monkeypatch.setenv("LHA_SANDBOX_CALLER_SA", CALLER_SA)
    monkeypatch.setenv("LHA_LOCAL_ROOT", str(tmp_path))
...
async def test_build_sandbox_raises_when_prod_and_missing_caller_sa(
    ...
    with pytest.raises(SandboxConfigurationError, match="LHA_SANDBOX_CALLER_SA"):
        await on_session_start_callback(_fake_context())
...
async def test_refresh_sandbox_auth_refreshes_routing_token_after_throttle(
    ...
    monkeypatch.setattr("horizon.environment.sandbox.time.monotonic", fake_monotonic)
