...
class TestFactory:
    async def test_default_returns_in_memory(self, monkeypatch):
        from horizon.scheduler.store import InMemoryReminderStore, get_reminder_store
    ...
    async def test_unknown_backend_raises(self, monkeypatch):
        from horizon.scheduler.store import get_reminder_store, reset_reminder_store
