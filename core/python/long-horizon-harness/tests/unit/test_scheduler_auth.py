...
async def test_dev_bypass_logs_warning(monkeypatch, caplog):
    ...
    assert any(
        "LHA_SCHEDULER_AUTH_DISABLED" in record.message for record in caplog.records
    )
...
async def test_missing_audience_fails_closed(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await verify_cloud_scheduler_token(_request_with_header("Bearer some-token"))
...
class TestAllowlist:
    async def test_email_on_allowlist_allows(self, monkeypatch):
        ...
        monkeypatch.setenv("LHA_SCHEDULER_AUDIENCE", "https://svc/")
        monkeypatch.setenv("LHA_SCHEDULER_SA", "scheduler@p.iam.gserviceaccount.com")
        monkeypatch.setattr(
            auth,
            "_verify_oauth2_token",
            lambda t, a: {"email": "scheduler@p.iam.gserviceaccount.com"},
        )
        await auth.verify_cloud_scheduler_token(_request_with_header("Bearer t"))
    ...
    async def test_missing_email_claim_returns_403(self, monkeypatch):
        ...
        monkeypatch.setenv("LHA_SCHEDULER_AUDIENCE", "https://svc/")
        monkeypatch.setenv("LHA_SCHEDULER_SA", "scheduler@p.iam.gserviceaccount.com")
        monkeypatch.setattr(auth, "_verify_oauth2_token", lambda t, a: {"sub": "123"})
        with pytest.raises(HTTPException) as exc_info:
            await auth.verify_cloud_scheduler_token(_request_with_header("Bearer t"))
        assert exc_info.value.status_code == 403
