...
def captured_spawn(monkeypatch):
    ...
    monkeypatch.setattr(agent_mod.SIBLING_AGENT_PLUGIN, "spawn_sibling", fake_spawn)
    return calls
...
async def test_module_exposes_entrypoints():
    from horizon.memory.flush_fork import FLUSH_FORK_ENABLED_ENV, spawn_flush_fork
