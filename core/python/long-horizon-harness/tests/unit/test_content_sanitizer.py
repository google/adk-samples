...
def test_json_inline_data_becomes_text_placeholder():
    ...
    content = Content(role="user", parts=[Part(text="see attached"), _json_part()])
...
def test_csv_and_octet_stream_are_degraded():
    ...
    parts = [
        ...
        Part(inline_data=Blob(mime_type="application/octet-stream", data=b"\x00")),
    ]
...
def test_function_response_part_passes_through():
    ...
    fr = Part(function_response=FunctionResponse(name="t", response={"ok": True}))
