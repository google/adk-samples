...
def _env(tmp_path, monkeypatch):
    ...
    env._working_dir = tmp_path  # ensure working_dir is set without initialize()
...
async def test_run_skill_script_still_asks():
    ...
    result = await permission_guard(
        tool=_Tool("run_skill_script"), args={"name": "policy"}, tool_context=ctx
    )
...
async def test_gws_delete_in_chain_still_asks():
    ...
    ctx = _Ctx()
    cmd = (
        "gws gmail messages list --params '{}' && gws drive files delete --params '{}'"
    )
    result = await permission_guard(
        tool=_Tool("terminal"), args={"command": cmd}, tool_context=ctx
    )
...
async def test_ask_dispatches_confirmation_first_pass():
    ctx = _Ctx()
    result = await permission_guard(
        tool=_Tool("terminal"), args={"command": "bq rm -t x.y"}, tool_context=ctx
    )
    assert result is not None and result.get("confirmation_required") is True
...
async def test_resume_none_payload_defaults_to_session_grant():
    ...
    assert result is None
    assert ctx.state[PERMISSION_GRANTS_STATE_KEY][0]["commandPrefix"] == ["bq rm"]


async def test_resume_always_persists_rule(_env):
    ctx = _Ctx(tool_confirmation=_TC(True, {"outcome": "proceed_always_and_save"}))
    result = await permission_guard(
        tool=_Tool("terminal"), args={"command": "bq rm x"}, tool_context=ctx
    )
    assert result is None
...
async def test_chained_command_with_unsafe_segment_asks():
    ctx = _Ctx()
    result = await permission_guard(
        tool=_Tool("terminal"), args={"command": "ls && bq rm x"}, tool_context=ctx
    )
    assert result is not None and result.get("confirmation_required") is True
    assert ctx._requested["payload"]["proposedRule"]["commandPrefix"] == ["bq rm"]
...
async def test_quoted_interpreter_body_not_shredded():
    ...
    ctx = _Ctx()
    cmd = 'sudo python3 -c "import json\nfor r in ranked:\n    print(r); x | y\n"'
    result = await permission_guard(
        tool=_Tool("terminal"), args={"command": cmd}, tool_context=ctx
    )
    assert result is not None and result.get("confirmation_required") is True
...
async def test_resume_ge_choice_label_records_session_grant():
    ctx = _Ctx(
        tool_confirmation=_TC(True, {"choice": "Yes — allow `bq rm` this session"})
    )
    result = await permission_guard(
        tool=_Tool("terminal"), args={"command": "bq rm x"}, tool_context=ctx
    )
...
async def test_resume_ge_choice_index_maps_to_outcome():
    ...
    ctx = _Ctx(tool_confirmation=_TC(True, {"choice": "2"}))  # index 2 = "Always allow"
    result = await permission_guard(
        tool=_Tool("terminal"), args={"command": "bq rm x"}, tool_context=ctx
    )
    assert result is None  # proceed (and persists — covered by overlay test elsewhere)
...
async def test_always_allow_persists_across_turns_via_interface(tmp_path):
    ...
    save = _Ctx(tool_confirmation=_TC(True, {"outcome": "proceed_always_and_save"}))
    first = await permission_guard(
...
async def test_yolo_does_not_bypass_deny():
    ...
    ctx.state[PERMISSION_GRANTS_STATE_KEY] = [
        {"toolName": "terminal", "commandPrefix": ["rm -rf"], "decision": "deny"}
    ]
    result = await permission_guard(
        tool=_Tool("terminal"), args={"command": "rm -rf /tmp/x"}, tool_context=ctx
    )
