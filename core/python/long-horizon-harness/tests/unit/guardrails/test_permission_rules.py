...
def test_parse_normalizes_gemini_fields():
    rule = parse_rule(
        {"toolName": ["a", "b"], "commandPrefix": "git status", "decision": "allow"}
    )
...
def test_tool_wildcard_and_name_match():
    ...
    decision, _ = resolve_decision(rules, tool_name="write_file", args={}, command=None)
    assert decision == "ask_user"


def test_command_prefix_is_token_boundary_aware():
    rules = [
        parse_rule({"toolName": "terminal", "commandPrefix": "ls", "decision": "allow"})
    ]
    allow, _ = resolve_decision(
        rules, tool_name="terminal", args={"command": "ls -la"}, command="ls -la"
    )
...
def test_last_match_wins():
    rules = [
        ...
        parse_rule(
            {"toolName": "terminal", "commandPrefix": "git", "decision": "allow"}
        ),
    ]
    decision, _ = resolve_decision(
        rules, tool_name="terminal", args={"command": "git log"}, command="git log"
    )
    assert decision == "allow"
...
def test_subagent_constraint(monkeypatch):
    rules = [
        parse_rule({"toolName": "terminal", "subagent": "worker", "decision": "allow"})
    ]
...
def test_default_rules_open_shell_and_benign_tools():
    ...
    assert {"*", "terminal", "process", "add_memory", "reminder", "reload"} <= names
...
async def test_interface_append_then_load_roundtrip(tmp_path: Path):
    ...
    await append_persisted_rule(env, {"toolName": "routine", "decision": "allow"})
...
async def test_interface_cache_does_not_leak_across_sandboxes(tmp_path: Path):
    ...
    await append_persisted_rule(alice, {"toolName": "routine", "decision": "allow"})
...
def test_session_grants_tolerate_none_state():
    ...
    assert write_session_grants(None, [{"toolName": "x", "decision": "allow"}]) is False
...
def test_parse_rule_trusted_skips_regex_safety():
    ...
    trusted_rule = parse_rule(
        {"toolName": "terminal", "commandRegex": complex_regex, "decision": "allow"},
        trusted=True,
    )
...
async def test_blanket_allow_for_terminal_is_dropped_from_overlay(monkeypatch):
    ...
    monkeypatch.setattr(
        "horizon.guardrails.permission_rules.load_persisted_rules", fake_persisted
    )
