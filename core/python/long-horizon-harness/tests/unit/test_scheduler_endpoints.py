...
class _FakeSessionService:
    ...
    async def list_sessions(self, *, app_name: str, user_id: str | None = None) -> Any:
        ...
        return SimpleNamespace(
            sessions=[
                s for s in self._sessions if user_id is None or s.user_id == user_id
            ]
        )
...
def _build_test_app(
    *,
    ...
    app.state.runner = SimpleNamespace(
        memory_service=object() if memory_service is _NO_MEMORY else memory_service,
...
class TestDreamReviewEndpoint:
    ...
    async def test_empty_user_ids_no_active_returns_zero(self):
        client = TestClient(_build_test_app(session_service=_FakeSessionService([])))
        response = client.post(
            "/scheduler/dream-review", json={"user_ids": [], "app_name": "lha"}
        )
        assert response.status_code == 200
    ...
    async def test_session_service_not_registered_returns_zero(self):
        ...
        response = TestClient(app).post(
            "/scheduler/dream-review", json={"user_ids": ["alice"], "app_name": "lha"}
        )
        body = response.json()
        assert body["reviewed"] == 0
...
class TestSnapshotEndpoint:
    async def test_disabled_is_noop(self, monkeypatch):
        ...
        client = TestClient(_build_test_app())
        response = client.post(
            "/scheduler/snapshot", json={"user_ids": ["alice"], "app_name": "lha"}
        )
        assert response.status_code == 200
        body = response.json()
    ...
    async def test_dispatches_per_user(self, monkeypatch):
        ...
        monkeypatch.setattr(provider, "_vertex_client_factory", lambda: object())
        monkeypatch.setattr(
        ...
        def fake_snapshot(client, *, agent_instance_name, user_id, snapshot_ttl, keep):
            seen.append(user_id)
