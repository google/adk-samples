...
@pytest.mark.asyncio
async def test_overflow_writes_full_text_and_returns_pointer(env_root: Path) -> None:
    big = "\n".join(f"row-{i:05d}" for i in range(5000))
    result = await overflow_to_file(big, stream="stdout", id_factory=lambda: "deadbeef")
...
async def test_pointer_reports_line_and_byte_counts(env_root: Path) -> None:
    ...
    result = await overflow_to_file(big, stream="stdout", id_factory=lambda: "z")
    n_lines = big.count("\n") + 1
