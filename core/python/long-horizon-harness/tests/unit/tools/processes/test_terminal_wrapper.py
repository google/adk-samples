...
from horizon.environment_context import clear_active_environment, set_active_environment
...
class TestForegroundUnchanged:
    ...
    async def test_default_path_spills_overflow_to_file(self, env_root: Path) -> None:
...
class TestBackgroundSpawn:
    async def test_background_returns_session_id_immediately(
        ...
        ctx = _ctx()
        result = await terminal(command="sleep 5", background=True, tool_context=ctx)
        assert result["status"] == "running"
        assert result["session_id"].startswith("proc_")
        assert result["pid"] > 0

    async def test_background_handle_lands_in_registry(self, env_root: Path) -> None:
        from horizon.tools.processes.process import process
        from horizon.tools.processes.terminal import terminal

        ctx = _ctx()
        spawn = await terminal(command="sleep 5", background=True, tool_context=ctx)
...
class TestAutoPromote:
    ...
    async def test_default_on_timeout_is_background(self, env_root: Path) -> None:
        from horizon.tools.processes.terminal import terminal
...
class TestNoInBodyGating:
    async def test_background_spawn_does_not_self_gate(self, env_root: Path) -> None:
...
class TestRegistrySharing:
    async def test_subsequent_calls_share_registry(self, env_root: Path) -> None:
        from horizon.tools.processes.process import process
        from horizon.tools.processes.terminal import terminal

        ctx = _ctx()
        s1 = await terminal(command="sleep 5", background=True, tool_context=ctx)
        s2 = await terminal(command="sleep 5", background=True, tool_context=ctx)
        listing = await process(action="list", tool_context=ctx)
        ids = {s["session_id"] for s in listing["running"]}
        assert s1["session_id"] in ids
        assert s2["session_id"] in ids

    async def test_cleanup_kills_running_processes(self, env_root: Path) -> None:
        ...
        ctx = _ctx()
        spawn = await terminal(command="sleep 30", background=True, tool_context=ctx)
