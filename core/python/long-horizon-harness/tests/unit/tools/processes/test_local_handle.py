...
def handle_factory(tmp_path: Path):
    ...
    def _make(
        command: str, *, buffer_bytes: int | None = None, cwd: Path | None = None
    ) -> LocalProcessHandle:
...
class TestExitAndWait:
    ...
    @pytest.mark.asyncio
    async def test_wait_no_timeout_blocks_until_exit(self, handle_factory) -> None:
...
class TestReadOutput:
    ...
    @pytest.mark.asyncio
    async def test_read_with_offset_skips_consumed_bytes(self, handle_factory) -> None:
