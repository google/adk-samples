...
class TestWalkSkillDirs:
    ...
    def test_both_dirs_missing_returns_empty(self, tmp_path: Path):
        ...
        result = walk_skill_dirs(user_dir=tmp_path / "u", builtin_dir=tmp_path / "b")
...
class TestBuildSkillToolset:
    ...
    def test_keeps_load_and_resource_tools(self, skill_roots: tuple[Path, Path]):
    ...
    async def test_process_llm_request_injects_catalog(
        ...
        await toolset.process_llm_request(tool_context=None, llm_request=_StubReq())
