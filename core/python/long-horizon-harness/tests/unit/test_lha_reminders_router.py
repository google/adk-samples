...
from horizon.scheduler.store import Reminder, get_reminder_store, reset_reminder_store
...
async def test_list_returns_user_reminders_sorted_by_fire_at(app_client):
    ...
    await store.add(_reminder("r2", "alice@x", datetime(2026, 6, 10, 9, tzinfo=UTC)))
    await store.add(_reminder("r1", "alice@x", datetime(2026, 6, 5, 9, tzinfo=UTC)))
...
async def test_list_excludes_other_users(app_client):
    store = get_reminder_store()
    await store.add(_reminder("mine", "alice@x", datetime(2026, 6, 5, tzinfo=UTC)))
    await store.add(_reminder("theirs", "bob@x", datetime(2026, 6, 5, tzinfo=UTC)))
    ids = [r["id"] for r in app_client.get("/lha/reminders").json()["reminders"]]
    assert ids == ["mine"]


async def test_delete_removes_own_reminder(app_client):
    store = get_reminder_store()
    await store.add(_reminder("r1", "alice@x", datetime(2026, 6, 5, tzinfo=UTC)))
