...
async def test_byte_identical_when_state_unchanged_within_day(tmp_path: Path):
    ...
    assert request_a.config.system_instruction == request_b.config.system_instruction, (
...
async def test_memory_guidance_absent_when_memory_tool_not_loaded(tmp_path: Path):
    stable = build_stable_tier(
...
async def test_skills_guidance_absent_when_skill_tool_not_loaded(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file", "terminal"],
        model_name="gemini-flash-latest",
        cwd=tmp_path,
...
async def test_artifact_html_guidance_absent_when_tool_not_loaded(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file", "terminal"],
        model_name="gemini-flash-latest",
        cwd=tmp_path,
...
async def test_google_operational_guidance_present_for_gemini_model(tmp_path: Path):
...
async def test_google_operational_guidance_absent_for_non_google_model(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
...
async def test_google_operational_guidance_absent_when_no_tools_loaded(tmp_path: Path):
...
async def test_tool_use_enforcement_present_for_enforcement_model(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
        model_name="gemini-flash-latest",
        cwd=tmp_path,
...
async def test_tool_use_enforcement_absent_for_non_enforcement_model(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
        model_name="claude-3-5-sonnet",
        cwd=tmp_path,
...
async def test_output_style_guidance_present_for_enforcement_model(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
        model_name="gemini-flash-latest",
        cwd=tmp_path,
...
async def test_output_style_guidance_absent_for_non_enforcement_model(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
        model_name="claude-3-5-sonnet",
        cwd=tmp_path,
...
async def test_tool_use_safety_guidance_present_for_enforcement_model(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
        model_name="gemini-flash-latest",
        cwd=tmp_path,
...
async def test_tool_use_safety_guidance_env_force_off(monkeypatch, tmp_path: Path):
    monkeypatch.setenv("LHA_TOOL_USE_ENFORCEMENT", "off")
...
async def test_tool_use_safety_guidance_env_force_on(monkeypatch, tmp_path: Path):
    monkeypatch.setenv("LHA_TOOL_USE_ENFORCEMENT", "on")
...
async def test_subagent_usage_guidance_present_for_enforcement_model(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
        model_name="gemini-flash-latest",
        cwd=tmp_path,
...
async def test_subagent_usage_guidance_absent_for_non_enforcement_model(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
        model_name="claude-3-5-sonnet",
        cwd=tmp_path,
...
async def test_subagent_usage_guidance_env_force_off(monkeypatch, tmp_path: Path):
    monkeypatch.setenv("LHA_TOOL_USE_ENFORCEMENT", "off")
...
async def test_subagent_usage_guidance_env_force_on(monkeypatch, tmp_path: Path):
    monkeypatch.setenv("LHA_TOOL_USE_ENFORCEMENT", "on")
...
async def test_planning_discipline_guidance_env_force_off(monkeypatch, tmp_path: Path):
    monkeypatch.setenv("LHA_TOOL_USE_ENFORCEMENT", "off")
...
async def test_planning_discipline_guidance_env_force_on(monkeypatch, tmp_path: Path):
    monkeypatch.setenv("LHA_TOOL_USE_ENFORCEMENT", "on")
...
async def test_failure_loop_guidance_present_for_enforcement_model(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
        model_name="gemini-flash-latest",
        cwd=tmp_path,
...
async def test_failure_loop_guidance_absent_for_non_enforcement_model(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
        model_name="claude-3-5-sonnet",
        cwd=tmp_path,
...
async def test_filesystem_write_guidance_present_for_enforcement_model(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
        model_name="gemini-flash-latest",
        cwd=tmp_path,
...
async def test_filesystem_write_guidance_env_force_off(monkeypatch, tmp_path: Path):
    monkeypatch.setenv("LHA_TOOL_USE_ENFORCEMENT", "off")
...
async def test_filesystem_write_guidance_env_force_on(monkeypatch, tmp_path: Path):
    monkeypatch.setenv("LHA_TOOL_USE_ENFORCEMENT", "on")
...
async def test_web_research_citation_guidance_present_when_tool_loaded(tmp_path: Path):
    stable = build_stable_tier(
...
async def test_session_search_guidance_absent_when_tool_not_loaded(tmp_path: Path):
    stable = build_stable_tier(
        tool_names=["read_file"],
        model_name="gemini-flash-latest",
        cwd=tmp_path,
...
async def test_environment_hint_includes_user_and_home(tmp_path: Path, monkeypatch):
...
async def test_environment_hint_includes_cli_versions(tmp_path: Path, monkeypatch):
...
async def test_environment_hint_cli_probe_is_cached(tmp_path: Path, monkeypatch):
...
async def test_stable_tier_cached_in_state_across_calls(tmp_path: Path):
    ...
    assert cached_after_first, "Stable tier must be cached in state after turn 1."
    assert request_a.config.system_instruction == request_b.config.system_instruction
