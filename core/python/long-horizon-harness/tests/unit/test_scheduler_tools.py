...
class TestListAndCancel:
    async def test_list_returns_only_caller(self):
        ...
        alice_view = await reminder(action="list", tool_context=_ctx(user_id="alice"))
        bob_view = await reminder(action="list", tool_context=_ctx(user_id="bob"))
    ...
    async def test_cancel_scoped_to_owner(self):
        ...
        # alice can
        ok = await reminder(action="cancel", id=rid, tool_context=_ctx(user_id="alice"))
        assert ok["success"] is True

    async def test_cancel_unknown_returns_false(self):
        from horizon.scheduler.tools import reminder

        result = await reminder(action="cancel", id="not-real", tool_context=_ctx())
        assert result["success"] is False
...
class TestDispatchErrors:
    async def test_schedule_missing_when_rejected(self):
        ...
        result = await reminder(action="schedule", message="x", tool_context=_ctx())
        assert result["success"] is False
        assert "when" in result["error"].lower()
