...
def _auth_config(
    *,
    ...
    return SimpleNamespace(
        ...
        auth_scheme=SimpleNamespace(flows=SimpleNamespace(authorizationCode=flow)),
...
def _event_with_request(fcid: str, *, config: SimpleNamespace) -> SimpleNamespace:
    return SimpleNamespace(
        content=None,
...
def test_latest_pending_wins_when_multiple_unresolved():
    events = [
        _event_with_request("fc-1", config=_auth_config(credential_key="github")),
        _event_with_request("fc-2", config=_auth_config(credential_key="google-drive")),
    ]
    pending = _latest_pending_credential(events)
