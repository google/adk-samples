...
def test_todos_ride_reminder_tail_not_instruction(env, tmp_path):
    ...
    from horizon.conversation.system_prompt import system_prompt_assembly_callback
    from horizon.tools.todos import write_todos

    asyncio.run(write_todos(todos=[{"content": "ship it", "status": "in_progress"}]))
...
def test_build_todos_reminder_renders_only_current_session(env):
    ...
    asyncio.run(
        reconcile_todos(
            [{"id": "s1", "content": "s1 task", "status": "pending"}], session_id="s1"
        )
    )
...
def test_reminder_callback_injects_current_session_board(env, tmp_path):
    ...
    rem_req = _reminder_request()
    asyncio.run(reminder_injection_callback(_session_ctx(tmp_path, "s1"), rem_req))
    tail = _reminder_texts(rem_req)
