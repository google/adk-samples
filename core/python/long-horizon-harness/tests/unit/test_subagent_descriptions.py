...
def _make_request(*tool_names: str):
    ...
    config = types.GenerateContentConfig(tools=[types.Tool(function_declarations=fds)])
...
async def test_rewrites_delegate_and_agent_with_skills(skills_home: Path) -> None:
    _write_skill(skills_home, "gws-drive", "Drive enumeration helper.")
    from horizon.subagents.descriptions import make_subagent_description_callback
...
async def test_empty_catalog_shows_placeholder(monkeypatch) -> None:
    ...
    monkeypatch.setattr(descriptions_mod, "load_session_skill_catalog", lambda: {})
