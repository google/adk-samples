...
def test_sign_verify_roundtrip():
    payload = {"user_id": "u1", "target": "workspace", "readonly": True, "nonce": "n"}
...
def test_exchange_code_returns_token_and_expiry():
    ...
    tok, exp = g.exchange_code(
        code="c", client_id="i", client_secret="s", redirect_uri="r", post=fake_post
    )
    assert tok == "ya29.tok"
    assert exp == 3599
...
async def test_persist_workspace_writes_token_meta():
    ...
    assert _json.loads(u[g.GWS_META_KEY]) == {"surfaces": ["chat"], "readonly": True}
...
def client_and_store(monkeypatch):
    ...
    monkeypatch.setenv("LHA_GCP_OAUTH_REDIRECT_URI", "https://h/lha/gcp/callback")
...
def test_callback_forged_state_400_no_write(client_and_store):
    ...
    r = client.get("/lha/gcp/callback", params={"code": "x", "state": "forged.sig"})
...
def test_callback_workspace_writes_gws_token_and_meta(client_and_store):
    client, store = client_and_store
    state = _state(
        user_id="u-test", target="workspace", surfaces=["chat", "drive"], readonly=True
    )
...
def test_connect_rejects_bad_target(client_and_store):
    ...
    assert client.post("/lha/gcp/connect", params={"target": "nope"}).status_code == 400


def test_connect_workspace_requires_a_surface(client_and_store):
    client, _ = client_and_store
    r = client.post("/lha/gcp/connect", params={"target": "workspace", "surfaces": ""})
    assert r.status_code == 400
