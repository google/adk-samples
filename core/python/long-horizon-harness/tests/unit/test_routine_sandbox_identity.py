...
def test_find_routine_sandbox_matches_running_routine():
    sandboxes = [
        ...
        _sandbox_stub("sb-stopped", "lhart-digest-v0_11_0", state="STATE_STOPPED"),
...
def test_user_and_routine_lookups_are_mutually_exclusive():
    ...
    assert (
        find_routine_sandbox(
            client, agent_instance_name=ENGINE, routine_id="digest", image_uri=IMAGE
        )
        == "sb-routine"
    )
    ...
    assert (
        find_routine_sandbox(
            client, agent_instance_name=ENGINE, routine_id="alice", image_uri=IMAGE
        )
        is None
    )
