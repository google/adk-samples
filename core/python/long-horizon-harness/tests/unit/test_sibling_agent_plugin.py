...
def _fake_runner(events: list[Any] | None = None, *, raise_exc: bool = False) -> Any:
...
class TestSpawnSiblingIdentifiers:
    async def test_session_created_under_parent_ids(self, monkeypatch):
        ...
        def fake_build_runner(
            *, agent: Any, app_name: str, session_service: Any, memory_service: Any
        ) -> Any:
        ...
        monkeypatch.setattr(sibling_agent_plugin, "_build_runner", fake_build_runner)
    ...
    async def test_passes_prompt_text_into_user_message(self, monkeypatch):
        ...
        runner = _fake_runner([])
        monkeypatch.setattr(sibling_agent_plugin, "_build_runner", lambda **_kw: runner)
...
class TestSpawnSiblingOnEvent:
    async def test_invokes_on_event_for_each_event(self, monkeypatch):
        ...
        monkeypatch.setattr(
            sibling_agent_plugin, "_build_runner", lambda **_kw: _fake_runner(events)
        )
...
class TestTaskTracking:
    async def test_task_tracked_then_released(self, monkeypatch):
        ...
        monkeypatch.setattr(
            sibling_agent_plugin, "_build_runner", lambda **_kw: _fake_runner([])
        )
...
class TestClose:
    ...
    async def test_close_inside_shorter_outer_timeout_does_not_raise(self, monkeypatch):
