...
@pytest.fixture
def local_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> LocalEnvironment:
...
async def test_save_reads_from_env_and_stores_in_context(
    ...
    result = await artifact(action="save", path="report.csv", tool_context=fake_ctx)
...
async def test_save_sets_display_name_for_a2a_filepart_name(
    ...
    (local_env.working_dir / "damped_oscillator.png").write_bytes(b"\x89PNG\r\nfake")
    await artifact(action="save", path="damped_oscillator.png", tool_context=fake_ctx)
...
async def test_load_writes_to_env(
    ...
    assert result["success"] is True
    assert (local_env.working_dir / "incoming" / "hello.txt").read_bytes() == payload
...
class TestDispatchErrors:
    ...
    async def test_load_missing_dest_path_rejected(
        ...
        result = await artifact(action="load", name="foo.txt", tool_context=fake_ctx)
        assert result["success"] is False
