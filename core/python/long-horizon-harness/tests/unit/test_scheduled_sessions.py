...
from horizon.scheduler.sessions import create_scheduled_session, scheduled_session_state
...
async def test_create_scheduled_session_persists_tag() -> None:
    ...
    full = await svc.get_session(app_name="lha", user_id="alice", session_id=session.id)
