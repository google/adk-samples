...
async def test_first_halt_turn_strips_tools_and_returns_none():
    ...
    response = await halt_consumer_callback(callback_context=ctx, llm_request=req)
...
async def test_halt_reason_set_returns_llm_response_containing_reason_verbatim():
    ...
    response = await halt_consumer_callback(callback_context=ctx, llm_request=object())
...
async def test_response_has_single_text_part():
    ...
    assert len(parts) == 1, f"Expected exactly 1 part, got {len(parts)}: {parts!r}"
...
async def test_clearing_halt_reason_between_calls_makes_second_call_pass_through():
    ...
    """
    state: dict[str, Any] = _halted_context_past_handoff("tool loop detected").state
    ctx = _fake_context(state)

    first = await halt_consumer_callback(callback_context=ctx, llm_request=object())
    assert first is not None, "First call (halt set, post-handoff) must short-circuit"

    state.pop(HALT_REASON_STATE_KEY)

    second = await halt_consumer_callback(callback_context=ctx, llm_request=object())
