...
class TestDefaultSeed:
    async def test_seed_blocks_rm_rf_root_via_terminal(self, policies_env: Path):
...
class TestSeedRootDeletionAndRcTruncation:
    ...
    async def test_seed_blocks_dangerous(self, policies_env: Path, command: str):
        from horizon.guardrails.policies import policies_guard
...
class TestDestructiveCommands:
    ...
    async def test_substring_match_is_case_insensitive(self, policies_env: Path):
        from horizon.guardrails.policies import policies_guard
...
class TestLoaderRobustness:
    ...
    async def test_ignores_missing_canonical_tool_name(self, policies_env: Path):
        from horizon.guardrails.policies import load_policies
...
class TestInterfaceLoader:
    async def test_sandbox_overlay_read_via_interface(self, tmp_path: Path):
        ...
        env.put(
            ...
            json.dumps({"canonical_tool_name": "frobnicate", "destructive": True}),
        )
...
class TestEdgeCases:
    ...
    async def test_no_active_environment_falls_back_to_defaults(self, monkeypatch):
        from horizon.guardrails import policies
...
async def test_echo_of_dangerous_string_not_blocked():
    ...
    res = await policies_guard(
        tool=_Tool("terminal"), args={"command": 'echo "rm -rf /"'}, tool_context=_Ctx()
    )
    assert res is None
...
async def test_block_message_names_source():
    ...
    res = await policies_guard(
        tool=_Tool("terminal"), args={"command": "rm -rf /"}, tool_context=_Ctx()
    )
