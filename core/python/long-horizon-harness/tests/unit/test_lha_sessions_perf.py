...
class _RecordingSessionService:
    inner: InMemorySessionService = field(default_factory=InMemorySessionService)
...
def test_list_passes_bounded_config_to_get_session() -> None:
    ...
    for cfg in runner.session_service.get_calls:
        assert isinstance(cfg, GetSessionConfig), "get_session called without a config"
        assert cfg.num_recent_events is not None, "config did not bound event load"
