...
def _write_skill(root: Path, name: str, body: str, description: str = "test") -> None:
...
async def test_build_child_agent_default_toolsets_used_when_empty(skills_home: Path):
    ...
    child = await build_child_agent(goal="x", context="", toolsets=[], skills=[])
...
async def test_build_child_agent_unknown_toolset_raises(skills_home: Path):
    ...
    with pytest.raises(KeyError):
        await build_child_agent(goal="x", context="", toolsets=["bogus"], skills=[])
...
async def test_build_child_agent_injects_skill_content(skills_home: Path):
    _write_skill(
        skills_home, "gws-drive", body="Run `gws drive files list` to enumerate."
    )
    from horizon.subagents.delegate_builder import build_child_agent
...
async def test_build_child_agent_skips_missing_skill_silently(skills_home: Path):
...
async def test_build_child_agent_no_memory_no_clarify_no_delegate_recursion(
    ...
    child = await build_child_agent(goal="x", context="", toolsets=["file"], skills=[])
...
async def test_build_child_agent_tools_param_merges_with_toolsets(skills_home: Path):
...
async def test_caller_instructions_appear_after_goal(skills_home: Path):
    ...
    assert context_at < caller_at, "Caller instructions must come after ## Context"
...
async def test_build_child_agent_custom_name_sanitized_and_suffixed(
    ...
    assert child.name.startswith("research_lru_cache_") or child.name.startswith(
        "research-lru-cache_"
    )
