...
class _Ctx:
    def __init__(self, *, user_id="alice@x", tool_confirmation=None):
        ...
        self._invocation_context = SimpleNamespace(user_id=user_id, app_name="app")
...
async def test_create_invalid_cron_rejected():
    ...
    res = await routine_tools.routine(
        action="create", name="x", schedule="not cron", task="t", tool_context=ctx
    )
...
async def test_create_declined_persists_nothing():
    ...
    res = await routine_tools.routine(
        action="create", name="Digest", schedule="0 8 * * *", task="t", tool_context=ctx
    )
...
async def test_list_and_cancel():
    ...
    await routine_tools.routine(
        action="create", name="D", schedule="0 8 * * *", task="t", tool_context=ctx
    )
