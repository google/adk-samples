...
from horizon.routines.manifest import DEFAULT_DELIVERY, RoutineManifest, parse_manifest
...
def test_defaults_for_secrets_and_delivery():
    m = parse_manifest({"name": "n", "schedule": "s", "task": "t"}, routine_id="x")
...
def test_missing_required_returns_none():
    assert parse_manifest({"name": "n", "schedule": "s"}, routine_id="x") is None


def test_blank_id_returns_none():
    assert (
        parse_manifest({"name": "n", "schedule": "s", "task": "t"}, routine_id="")
        is None
    )
...
def test_secrets_coercion_drops_non_strings_and_blanks():
    m = parse_manifest(
        {"name": "n", "schedule": "s", "task": "t", "secrets": ["A", 5, "  ", "B"]},
        routine_id="x",
    )
    assert m is not None
