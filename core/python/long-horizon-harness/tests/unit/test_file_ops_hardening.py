...
class TestReadFileBlocksNonRegularFiles:
    ...
    async def test_read_fifo_returns_error_not_hang(self, tmp_path: Path) -> None:
...
class TestReadFileCapsContentLength:
    ...
    async def test_small_file_is_not_marked_truncated(self, tmp_path: Path) -> None:
...
class TestReadFileNonUtf8:
    async def test_non_utf8_bytes_are_replaced_not_raised(self, tmp_path: Path) -> None:
...
class TestReadFileOffsetParity:
    async def test_offset_zero_equivalent_to_offset_one(self, tmp_path: Path) -> None:
...
class TestSearchFilesNoSymlinkFollow:
    ...
    async def test_search_does_not_traverse_symlink_outside_root(
        ...
        inside.mkdir()
        outside.mkdir()
        (outside / "secrets.txt").write_text("UNIQUE_MARKER_SHOULD_NOT_BE_FOUND\n")
