...
async def test_spawn_returns_task_id_and_status(monkeypatch):
    ...
    result = await agent(action="result", task_id=task_id, wait=True, timeout_s=5.0)
...
async def test_result_no_wait_returns_pending(monkeypatch):
    ...
    release.set()
    await agent(action="result", task_id=spawn["task_id"], wait=True, timeout_s=5.0)
    ...
    async def _fake_run(
        *, child: Any, user_message: str, timeout_s: float = 120.0
        ...
        return {
            ...
            "telemetry": {"iterations": 1, "duration_ms": 1, "invocation_id": "x"},
        }
...
async def test_wait_dispatch_returns_finished(monkeypatch):
    ...
    async def _fake_run(**_kwargs: Any) -> dict[str, Any]:
        return {"status": "completed", "summary": "waited-done", "telemetry": {}}
...
class TestSpawnWiderApi:
    ...
    async def test_spawn_custom_name_appears_in_list_and_status(self, monkeypatch):
