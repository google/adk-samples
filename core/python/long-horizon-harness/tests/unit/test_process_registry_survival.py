...
def test_survival_through_real_context_path_across_turns() -> None:
    ...
    ctx2 = _ctx("sess-1")
    found = ProcessRegistry.for_session(session_key_from_context(ctx2)).get("proc_1")
    assert found is not None

    other = _ctx("sess-2")
    assert (
        ProcessRegistry.for_session(session_key_from_context(other)).get("proc_1")
        is None
    )
...
def test_cap_keeps_running_session_registry_even_when_over_cap() -> None:
    ...
    ProcessRegistry.for_session("overflow")
    assert ProcessRegistry.for_session("running-old").get("proc-running") is not None
