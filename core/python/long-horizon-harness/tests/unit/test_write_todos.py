...
def test_tool_context_scopes_to_session_folder(env, tmp_path):
    ...
    assert [t["content"] for t in _run(read_todos(session_id="s1"))] == ["scoped"]
...
def test_returns_rendered_view(env):
    ...
    result = _run(write_todos(todos=[{"content": "do it", "status": "pending"}]))
...
def test_invalid_priority_rejected(env):
    ...
    result = _run(
        write_todos(todos=[{"content": "x", "status": "pending", "priority": "urgent"}])
    )
    assert result["success"] is False
...
def test_single_in_progress_no_warning(env):
    ...
    result = _run(write_todos(todos=[{"content": "a", "status": "in_progress"}]))
