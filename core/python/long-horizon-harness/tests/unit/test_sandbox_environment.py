...
class TestExecute:
    ...
    async def test_http_5xx_surfaces_nonzero_exit(self) -> None:
        ...
        assert result.exit_code != 0
        assert (
            "internal server error" in result.stderr.lower() or "500" in result.stderr
        )
...
class TestFileIO:
    ...
    async def test_read_file_handles_binary(self) -> None:
        ...
        with respx.mock() as mock:
            mock.get(f"{BASE_URL}/files").mock(
                return_value=httpx.Response(
                    200, json={"content_b64": base64.b64encode(payload).decode("ascii")}
                )
            )
    ...
    async def test_read_file_raises_file_not_found_on_404(self) -> None:
        ...
        with respx.mock() as mock:
            mock.get(f"{BASE_URL}/files").mock(
                return_value=httpx.Response(
                    404, json={"detail": "[Errno 2] No such file: '/missing.txt'"}
                )
            )
