...
async def _consume_bare_halt(plugin: IterationBudgetPlugin, state: dict[str, Any]):
...
async def test_on_event_does_not_replace_function_call_event_on_overflow():
    ...
    """
    plugin = IterationBudgetPlugin(max_iterations=10, max_tool_calls_per_iteration=2)
    ctx = _fake_invocation_context()
