...
def fake_pool(monkeypatch):
    ...
    monkeypatch.setattr(ps.PostgresReminderStore, "_ensure_pool", _fake_ensure_pool)
    return pool
...
class TestClaimDueSqlShape:
    async def test_claim_due_uses_skip_locked_and_deletes_one_shot(self, fake_pool):
        from horizon.scheduler.postgres_store import PostgresReminderStore
        ...
        delete_calls = [c for c in fake_pool.execute_calls if "DELETE" in c[0].upper()]
    ...
    async def test_claim_due_advances_recurring(self, fake_pool):
        ...
        update_calls = [c for c in fake_pool.execute_calls if "UPDATE" in c[0].upper()]
        assert len(update_calls) == 1
        next_at, rid = update_calls[0][1]
        assert rid == "r1"
        assert next_at == _at(2026, 5, 20, 9) + timedelta(days=1)

    async def test_claim_due_drops_rows_with_invalid_recurrence(self, fake_pool):
        from horizon.scheduler.postgres_store import PostgresReminderStore
