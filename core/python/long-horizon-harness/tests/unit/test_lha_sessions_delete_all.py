...
def test_delete_all_removes_only_current_users_sessions(runner: _StubRunner) -> None:
    import asyncio
    ...
    remaining_for_other = asyncio.run(
        runner.session_service.list_sessions(app_name=APP_NAME, user_id=OTHER_USER)
    ).sessions
...
def test_delete_all_continues_past_individual_failures(
    ...
    async def flaky_delete(*, app_name: str, user_id: str, session_id: str) -> None:
        if session_id == bad_id:
            raise RuntimeError("simulated backend failure")
        await real_delete(app_name=app_name, user_id=user_id, session_id=session_id)
