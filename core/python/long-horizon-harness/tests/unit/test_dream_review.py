    ...
    return Event(
        ...
        content=Content(
            ...
            parts=[
                ...
                Part(inline_data=Blob(mime_type="application/json", data=b'{"a":1}')),
            ],
        ),
        invocation_id="inv-test",
    )
...
async def test_generate_receives_no_unsupported_inline_mime(fake_vertex):
    ...
    assert any(
        getattr(p, "text", None) and "config" in p.text for p in sent[0].content.parts
    )
...
class _FakeSessionService:
    ...
    def __init__(self, sessions: list[list[Event]], *, user_id: str = "user-1") -> None:
        self._sessions = {
            f"s{i}": SimpleNamespace(
                id=f"s{i}", user_id=user_id, last_update_time=float(i), events=events
            )
    ...
    async def list_sessions(self, *, app_name: str, user_id: str | None = None) -> Any:
        ...
        return SimpleNamespace(
            sessions=[
                SimpleNamespace(
                    id=s.id, user_id=s.user_id, last_update_time=s.last_update_time
                )
...
class _FakeMemories:
    ...
    async def generate(
        self, *, name, direct_contents_source=None, scope=None, config=None, **_kw
    ) -> Any:
        self.generate_calls.append(
            {
                "name": name,
                "scope": scope,
                "events": list(getattr(direct_contents_source, "events", []) or []),
                "config": config,
            }
        )
        generated = [
            SimpleNamespace(action=SimpleNamespace(name=a)) for a in self._actions
        ]
        return SimpleNamespace(
...
async def test_returns_failure_when_no_session_content(fake_vertex):
    ...
    result = await dream_review._run_dream_review_for_user(
        ...
        app_name="parent",
        user_id="user-1",
        session_service=_FakeSessionService([[], []]),  # sessions, but no events
    )
    assert result["success"] is False
...
async def test_generate_called_with_scope_and_events(fake_vertex):
    ...
    service = _FakeSessionService([[_make_event("first")], [_make_event("second")]])
    result = await dream_review._run_dream_review_for_user(
        memory_service=object(),
        app_name="parent",
        user_id="user-1",
