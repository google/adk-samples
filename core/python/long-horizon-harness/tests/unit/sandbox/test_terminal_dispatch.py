...
@pytest.mark.asyncio
@respx.mock
async def test_background_spawn_calls_runtime_processes_endpoint(sandbox_env) -> None:
    from horizon.tools.processes.terminal import terminal

    route = respx.post(f"{BASE_URL}/processes").mock(
        return_value=httpx.Response(200, json={"session_id": "proc_sand1", "pid": 9999})
    )

    result = await terminal(command="sleep 30", background=True, tool_context=_ctx())
...
async def test_background_spawn_registers_handle_for_process_tool(
    ...
    respx.post(f"{BASE_URL}/processes").mock(
        return_value=httpx.Response(200, json={"session_id": "proc_sand2", "pid": 1})
    )
...
async def test_process_tool_kill_routes_to_runtime(sandbox_env) -> None:
    ...
    respx.post(f"{BASE_URL}/processes").mock(
        return_value=httpx.Response(200, json={"session_id": "proc_sand3", "pid": 1})
    )
    ...
    ctx = _ctx()
    await terminal(command="sleep 30", background=True, tool_context=ctx)
    result = await process(action="kill", session_id="proc_sand3", tool_context=ctx)
...
async def test_process_tool_log_decodes_remote_output(sandbox_env) -> None:
    ...
    respx.post(f"{BASE_URL}/processes").mock(
        return_value=httpx.Response(200, json={"session_id": "proc_sand4", "pid": 1})
    )
    ...
    ctx = _ctx()
    await terminal(command="echo line-from-sandbox", background=True, tool_context=ctx)
