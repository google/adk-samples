...
RUNTIME_DIR = Path(__file__).resolve().parents[3] / "horizon" / "sandbox" / "runtime"
...
def _wait_until_finished(client: TestClient, sid: str, deadline: float = 5.0) -> dict:
...
class TestSpawnAndStatus:
    ...
    def test_status_reports_running_then_finished(self, client: TestClient) -> None:
    ...
    def test_status_unknown_session_returns_404(self, client: TestClient) -> None:
...
class TestOutput:
    def test_output_captures_stdout(self, client: TestClient) -> None:
        ...
        assert r.status_code == 200
        body = r.json()
        decoded = base64.b64decode(body["data_b64"]).decode("utf-8", errors="replace")
...
class TestKill:
    ...
    def test_kill_finished_process_is_idempotent(self, client: TestClient) -> None:
...
class TestStdin:
    def test_stdin_write_then_read(self, client: TestClient) -> None:
        ...
        r = client.post(
            ...
            json={"data_b64": base64.b64encode(b"hi").decode("ascii"), "submit": True},
        )
        assert r.status_code == 200, r.text
        _wait_until_finished(client, sid)
        out = client.get(f"/processes/{sid}/output").json()
        decoded = base64.b64decode(out["data_b64"]).decode("utf-8", errors="replace")
        assert "got=hi" in decoded

    def test_stdin_to_finished_process_returns_409(self, client: TestClient) -> None:
        sid = _spawn(client, "true")["session_id"]
        _wait_until_finished(client, sid)
        r = client.post(
            f"/processes/{sid}/stdin",
