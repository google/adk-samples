...
_SIGNED = "https://storage.googleapis.com/b/o/file.html/0?X-Goog-Signature=deadbeef"
...
def _content(name: str, response: dict) -> Content:
    return Content(
        ...
        parts=[
            Part(
                function_response=FunctionResponse(id="x", name=name, response=response)
            )
        ],
    )
...
def test_artifact_save_url_is_redacted():
    c = _content("artifact", {"success": True, "filename": "p.html", "url": _SIGNED})
...
def test_artifact_load_without_url_is_noop():
    c = _content("artifact", {"success": True, "filename": "p.html", "dest_path": "/x"})
    assert redact_artifact_urls([c]) == 0
