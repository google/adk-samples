...
@pytest.fixture
def env_at_tmp(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> LocalEnvironment:
...
def test_upload_happy_path_writes_file(env_at_tmp: LocalEnvironment) -> None:
    ...
    assert (env_at_tmp.working_dir / "uploads" / "dataset.csv").read_bytes() == payload
...
def test_upload_oversize_is_413(
    ...
    resp = client.post(
        "/lha/uploads",
        files={"file": ("big.bin", io.BytesIO(b"x" * 32), "application/octet-stream")},
    )
...
def test_upload_path_traversal_is_400(env_at_tmp: LocalEnvironment, name: str) -> None:
    client = TestClient(_build_app())
    resp = client.post(
        "/lha/uploads",
...
def test_upload_with_relpath_preserves_tree(env_at_tmp: LocalEnvironment) -> None:
    client = TestClient(_build_app())
    resp = client.post(
        "/lha/uploads",
        files={"file": ("main.py", io.BytesIO(b"print('hi')"), "text/x-python")},
...
def test_upload_with_relpath_accepts_backslashes(env_at_tmp: LocalEnvironment) -> None:
    client = TestClient(_build_app())
    resp = client.post(
        "/lha/uploads",
    ...
    assert resp.status_code == 200, resp.text
    assert (env_at_tmp.working_dir / "uploads" / "myproj" / "src" / "main.py").exists()
...
def test_upload_overwrite_reports_replaced_true(env_at_tmp: LocalEnvironment) -> None:
    client = TestClient(_build_app())
    client.post(
        "/lha/uploads",
...
def test_tree_happy_path_lists_files_and_dirs(env_at_tmp: LocalEnvironment) -> None:
...
def test_tree_empty_workspace_returns_empty(env_at_tmp: LocalEnvironment) -> None:
    client = TestClient(_build_app())
    resp = client.get("/lha/workspace/tree")
    assert resp.status_code == 200
    assert resp.json() == {"path": "/workspace", "entries": [], "truncated": False}


def test_tree_path_outside_workspace_is_400(env_at_tmp: LocalEnvironment) -> None:
    client = TestClient(_build_app())
    resp = client.get("/lha/workspace/tree", params={"path": "/etc"})
    assert resp.status_code == 400


def test_tree_subdir_returns_only_its_entries(env_at_tmp: LocalEnvironment) -> None:
...
def test_download_happy_path_returns_file_bytes(env_at_tmp: LocalEnvironment) -> None:
    payload = b"name,age\nalice,30\nbob,40\n"
...
def test_download_default_remains_attachment(env_at_tmp: LocalEnvironment) -> None:
...
def test_download_directory_returns_zip(env_at_tmp: LocalEnvironment) -> None:
    ...
    client = TestClient(_build_app())
    resp = client.get("/lha/workspace/download", params={"path": "/workspace/subdir"})
    assert resp.status_code == 200, resp.text
...
def test_download_outside_workspace_is_400(env_at_tmp: LocalEnvironment) -> None:
    client = TestClient(_build_app())
...
def test_download_oversize_is_413(
    ...
    client = TestClient(_build_app())
    resp = client.get("/lha/workspace/download", params={"path": "/workspace/big.bin"})
    assert resp.status_code == 413
    # Body must NOT contain the file bytes.
    assert b"x" * 32 not in resp.content


def test_download_unicode_filename_round_trips(env_at_tmp: LocalEnvironment) -> None:
    name = "résumé.csv"
    (env_at_tmp.working_dir / name).write_bytes(b"col\nval\n")
    client = TestClient(_build_app())
    resp = client.get("/lha/workspace/download", params={"path": f"/workspace/{name}"})
    assert resp.status_code == 200, resp.text
    disp = resp.headers["content-disposition"]
...
def test_delete_directory_without_recursive_is_400(
    ...
    (env_at_tmp.working_dir / "sub").mkdir()
    client = TestClient(_build_app())
    resp = client.delete("/lha/workspace/file", params={"path": "/workspace/sub"})
    assert resp.status_code == 400
    assert "recursive" in resp.json()["detail"].lower()
    assert (env_at_tmp.working_dir / "sub").is_dir()


def test_delete_directory_recursive_removes_tree(env_at_tmp: LocalEnvironment) -> None:
    sub = env_at_tmp.working_dir / "sub"
    sub.mkdir()
...
def test_delete_recursive_workspace_root_is_400(env_at_tmp: LocalEnvironment) -> None:
    client = TestClient(_build_app())
    resp = client.delete(
        "/lha/workspace/file",
