...
import horizon.feedback.sink as sink
...
def test_emit_falls_back_to_stdlib_logger_when_sink_raises(monkeypatch, caplog) -> None:
    boom = MagicMock()
    boom.log_struct.side_effect = RuntimeError("quota exceeded")
    monkeypatch.setattr(sink, "_get_cloud_logger", lambda: boom)
    record = {"log_type": "feedback", "origin": "user", "text": "please add dark mode"}
...
def test_emit_does_not_propagate_when_logger_build_fails(monkeypatch) -> None:
    monkeypatch.setattr(sink, "_cloud_logger", None)
    monkeypatch.setattr(
        sink, "_build_cloud_logger", MagicMock(side_effect=RuntimeError("no creds"))
    )
    # Should swallow the build error and fall back, not raise.
    sink.emit_feedback_record({"log_type": "feedback", "origin": "user", "text": "hi"})
