...
class TestTryClaim:
    ...
    def test_env_invalid_falls_back_to_default(
        ...
        state: dict[str, Any] = {}
        assert _throttle.try_claim(state, "auto_capture") is True
        assert _throttle.try_claim(state, "auto_capture") is False  # default 120s

    def test_state_is_none_passes_through(self, fake_clock: dict[str, float]) -> None:
    ...
    def test_corrupt_per_type_entry_is_treated_as_fresh(
        ...
        state: dict[str, Any] = {"_fork_throttle": {"auto_capture": "not_a_dict"}}
        assert _throttle.try_claim(state, "auto_capture") is True
        assert state["_fork_throttle"]["auto_capture"]["count"] == 1
...
class TestAutoCaptureThrottle:
    async def test_first_turn_flushes(self, fake_clock: dict[str, float]) -> None:
