...
@pytest.mark.asyncio
async def test_warm_returns_202_and_status(counted_local_env: dict[str, int]) -> None:
    app = _build_app()
...
@pytest.mark.asyncio
async def test_warm_is_safe_to_call_twice(counted_local_env: dict[str, int]) -> None:
    app = _build_app()
    transport = ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
...
def test_status_returns_idle_before_any_warm(reset_provisioning_state: None) -> None:
    app = _build_app()
    client = TestClient(app)
