...
def test_streamed_final_aggregate_is_suppressed():
    ...
    final = conv(_text_event("Hey! there", partial=False), artifacts, "t1", "c1")
...
def test_nonstreamed_single_event_is_kept():
    ...
    only = conv(_text_event("All at once.", partial=False), artifacts, "t1", "c1")
...
def test_streamed_final_drops_text_but_keeps_function_call():
    ...
    final = conv(_text_and_call_event("I'll load the skill."), artifacts, "t1", "c1")
...
def _function_call_in_artifact(a2a_events) -> bool:
    return any(
        a2a_compat.is_data_part(p) for ev in a2a_events for p in _artifact_parts(ev)
    )
...
def test_function_call_surfaces_as_status_message_not_artifact():
    ...
    assert any(
        any(a2a_compat.is_data_part(p) for p in m.parts) for m in _status_messages(out)
...
def test_tool_call_surfaces_as_datapart_without_label():
    ...
    assert _msg_datas(out), "function_call must ride a status message as a DataPart"
...
def test_fake_attachment_link_stripped_to_text():
    ...
    out = _strip_fake_artifact_links(
        [_text_artifact_event("Here's your plot: [plot.png](attachment://plot.png)")]
    )
...
def test_real_https_link_preserved():
    ...
    out = _strip_fake_artifact_links([_text_artifact_event(f"[plot.png]({url})")])
...
def test_append_after_seen_create_is_preserved():
    ...
    conv._enforce_artifact_lifecycle([create], "t1")  # create seen (append=False)
