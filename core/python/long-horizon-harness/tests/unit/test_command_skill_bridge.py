...
def _request(text: str) -> SimpleNamespace:
    return SimpleNamespace(contents=[Content(role="user", parts=[Part(text=text)])])
...
async def test_builtin_short_circuits(monkeypatch):
    ...
    monkeypatch.setattr(disp, "bound_skill_names", lambda: set())
...
async def test_image_only_turn_falls_through(monkeypatch):
    ...
    req = SimpleNamespace(
        contents=[
            Content(
                role="user",
                parts=[Part(inline_data=Blob(mime_type="image/png", data=b"\x89PNG"))],
            )
        ]
    )
