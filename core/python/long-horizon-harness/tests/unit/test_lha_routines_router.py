...
async def test_list_returns_user_routines_sorted_by_next_fire_at(app_client):
    ...
    await store.add(_routine("r2", "alice@x", datetime(2026, 6, 10, 9, tzinfo=UTC)))
    await store.add(_routine("r1", "alice@x", datetime(2026, 6, 5, 9, tzinfo=UTC)))
...
async def test_list_excludes_other_users(app_client):
    store = get_routine_store()
    await store.add(_routine("mine", "alice@x", datetime(2026, 6, 5, tzinfo=UTC)))
    await store.add(_routine("theirs", "bob@x", datetime(2026, 6, 5, tzinfo=UTC)))
