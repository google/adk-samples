...
def test_parse_marker_user():
    assert parse_marker("[user] I prefer espresso") == ("user", "I prefer espresso")
...
async def _seed_user_turn(service: InMemoryMemoryService, text: str) -> None:
    ...
    await service.add_events_to_memory(
        app_name=APP_NAME, user_id=USER_ID, events=[event], session_id=SESSION_ID
    )
...
class _FakePager:
    ...
    def __aiter__(self):
        raise AssertionError("next_page() is broken; _walk_vertex must read .page")
...
async def test_walk_vertex_reads_first_page_without_paginating():
    ...
    items = [
        _FakeMemory(fact="[user] one", update_time=datetime(2026, 5, 1, tzinfo=UTC)),
        _FakeMemory(fact="[user] two", update_time=datetime(2026, 5, 2, tzinfo=UTC)),
    ]
    service = _FakeVertexMemoryService(items)
...
async def test_walk_vertex_includes_unmarked_facts_as_user():
    items = [
        _FakeMemory(fact="I like Paris.", update_time=datetime(2026, 5, 1, tzinfo=UTC)),
        _FakeMemory(
