...
@pytest.fixture()
def wired_store():
    store = SecretManagerStore(client=FakeSecretClient(), project_id="proj", ttl_s=60.0)
...
@pytest.mark.asyncio
async def test_secret_env_uses_environment_owner(wired_store, tmp_path, monkeypatch):
