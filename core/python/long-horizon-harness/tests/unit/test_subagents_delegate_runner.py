...
async def test_run_delegate_concatenates_text_parts(
    ...
    result = await delegate_runner.run_delegate(child=stub_child, user_message="x")
...
async def test_run_delegate_records_duration(monkeypatch, stub_child: Agent) -> None:
    from horizon.subagents import delegate_runner
