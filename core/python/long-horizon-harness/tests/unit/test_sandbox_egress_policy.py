...
def _template_client() -> tuple[Any, dict[str, list[dict[str, Any]]]]:
    ...
    client = MagicMock()
    client.agent_engines.sandboxes.templates.list = lambda *, name, config=None: iter(
        []
    )
...
class TestDefaultInternetAccess:
    ...
    def test_dev_defaults_to_internet_on(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_prod_defaults_to_hermetic(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestEnsureTemplateEgress:
    def test_default_is_hermetic(self) -> None:
        from horizon.sandbox.lifecycle import ensure_template, template_display_name

        client, calls = _template_client()
        ensure_template(client, agent_instance_name=AGENT_INSTANCE, image_uri=IMAGE_URI)
