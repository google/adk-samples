...
class _ScriptedLlm(BaseLlm):
    ...
    async def generate_content_async(
        ...
        if advanced:
            yield LlmResponse(
                content=types.Content(role="model", parts=[types.Part(text="ADVANCED")])
            )
            return
        yield LlmResponse(
...
def _texts(events) -> list[str]:
    return [
        p.text for e in events for p in (e.content.parts if e.content else []) if p.text
    ]
        ...
        parts=[
            types.Part(
                function_response=types.FunctionResponse(
                    ...
                    response=ToolConfirmation(confirmed=True, payload={}).model_dump(),
                )
            )
        ],
    )
...
async def test_partial_approval_advances_without_running_pending_tool() -> None:
    ...
    runner = Runner(
        app=App(name=APP, root_agent=agent), session_service=ss, memory_service=ms
    )
