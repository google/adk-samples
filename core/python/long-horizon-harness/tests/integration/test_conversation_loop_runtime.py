...
async def test_plugin_short_circuits_when_budget_pre_halted() -> None:
    ...
    halt = events[-1]
    halted_text = " ".join(
        part.text for part in (halt.content.parts if halt.content else []) if part.text
    )
...
async def test_plugin_state_survives_between_runs() -> None:
...
async def test_plugin_isolates_budgets_per_session() -> None:
    ...
    session_a = await session_service.create_session(app_name=APP_NAME, user_id="alice")
    session_b = await session_service.create_session(app_name=APP_NAME, user_id="bob")
