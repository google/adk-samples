...
async def test_wrong_audience_returns_401(monkeypatch):
    ...
    monkeypatch.setattr(auth_mod, "_verify_oauth2_token", mimic_audience_mismatch)
...
async def test_email_not_on_allowlist_returns_403(monkeypatch):
    ...
    resp = await _post(_build_app(), {"Authorization": "Bearer valid-but-wrong-caller"})
...
async def test_happy_path_returns_200(monkeypatch):
    ...
    resp = await _post(_build_app(), {"Authorization": "Bearer cloud-scheduler-oidc"})
