...
async def test_cancel_mid_flight_returns_cancelled(registry):
    ...
    task_id = await registry.register(coro_factory=slow_factory, goal="never-completes")
...
async def test_parent_cancel_cascades_to_children(registry):
    ...
    parent_id = await registry.register(coro_factory=parent_factory, goal="parent")
