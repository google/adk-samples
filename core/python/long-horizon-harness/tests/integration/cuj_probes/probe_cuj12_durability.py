...
async def test_reminder_survives_pool_restart(monkeypatch, clean_table):
    ...
    rid = await revision_a.add(_reminder(fire_at=past, message="survives restart"))
    assert rid
    ...
    try:
        ...
        assert remaining == [], "one-shot must be gone after firing on new revision"
    finally:
...
@_skip_no_db
async def test_recurring_reminder_persists_advanced_fire_at(monkeypatch, clean_table):
