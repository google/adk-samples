...
async def delegate(tool_context: ToolContext) -> dict[str, Any]:
    ...
    if confirmation is None:
        ...
        if pending is not None:
            ...
            tool_context.request_confirmation(
                hint=f"[child] {tc.get('hint', 'approve?')}", payload=tc.get("payload")
            )
...
async def test_nested_handoff_runs_child_side_effect_exactly_once() -> None:
    ...
    await parent_sessions.create_session(app_name="parent", user_id="u", session_id="s")
    ...
    async for event in parent_runner.run_async(
        user_id="u",
        session_id="s",
        new_message=types.Content(role="user", parts=[types.Part(text="do it")]),
    ):
        run1.append(event)

    assert SIDE_EFFECTS == ["child:A", "child:B:req"], SIDE_EFFECTS
    parent_conf = _confirmation_call(run1)
    assert parent_conf is not None, "parent turn should have paused for confirmation"
