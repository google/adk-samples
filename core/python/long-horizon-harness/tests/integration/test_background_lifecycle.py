...
async def test_auto_promote_on_timeout_preserves_session() -> None:
    ...
    final = await process(action="wait", session_id=sid, timeout_s=10, tool_context=ctx)
...
async def test_write_unblocks_interactive_session() -> None:
    ...
    final = await process(action="wait", session_id=sid, timeout_s=5, tool_context=ctx)
    assert final["exit_code"] == 0
...
async def test_registry_isolated_per_session() -> None:
    ...
    spawn_a = await terminal(command="sleep 30", background=True, tool_context=ctx_a)
    spawn_b = await terminal(command="sleep 30", background=True, tool_context=ctx_b)
