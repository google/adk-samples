...
def test_clarify_request_confirmation_payload_round_trips_through_lha_state() -> None:
    ...
    first = clarify(question="continue?", choices=["yes", "no"], tool_context=ctx)
    ...
    event = SimpleNamespace(
        content=None,
        actions=SimpleNamespace(requested_tool_confirmations=requested_confirmations),
    )
...
def test_clarify_resume_with_tool_confirmation_short_circuits_request() -> None:
    ...
    ctx = SimpleNamespace(
        state={},
        tool_confirmation=SimpleNamespace(confirmed=True, payload={"answer": "yes"}),
        actions=SimpleNamespace(skip_summarization=False),
        request_confirmation=fake_request_confirmation,
    )

    result = clarify(question="continue?", choices=["yes", "no"], tool_context=ctx)
...
async def _commit_state(session_service: Any, session: Any, keys: list[str]) -> None:
    ...
    """
    state_delta = {key: session.state[key] for key in keys if key in session.state}
    if not state_delta:
        return
    event = Event(
...
async def test_auto_capture_throttle_state_persists_across_session_round_trip(
    ...
    session = await session_service.create_session(app_name="parent", user_id="alice")
