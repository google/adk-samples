...
async def _make_parent(state: dict[str, Any] | None = None) -> tuple[Runner, str, str]:
    app = App(
        name="parent",
        root_agent=LlmAgent(
...
@pytest.mark.asyncio
async def test_child_risky_op_surfaces_and_resumes_on_approval(monkeypatch) -> None:
    effects: list[str] = []
    ...
    assert parent_conf is not None, "the child's risky op should surface to the user"
        ...
        parts=[
            types.Part(
                function_response=types.FunctionResponse(
                    id=parent_conf.id, name=_CONFIRM_FN, response={"confirmed": True}
                )
            )
        ],
    )
...
async def test_child_risky_op_denied_on_decline(monkeypatch) -> None:
    ...
    decline = types.Content(
        ...
        parts=[
            types.Part(
                function_response=types.FunctionResponse(
                    id=parent_conf.id, name=_CONFIRM_FN, response={"confirmed": False}
                )
            )
        ],
    )
...
class _AskParentChildLlm(BaseLlm):
    async def generate_content_async(
        ...
        if "ask_parent" not in _called(llm_request.contents):
            ...
            yield _text(f"used region {_ask_parent_answer(llm_request.contents)}")
...
@pytest.mark.asyncio
async def test_child_ask_parent_bubbles_and_resumes_with_free_text(monkeypatch) -> None:
    monkeypatch.setattr(delegate_mod, "_build_child", _ask_parent_child_factory())
    runner, u, s = await _make_parent()
    ...
    assert conf is not None, "ask_parent should bubble the question to the parent turn"
        ...
        parts=[
            types.Part(
                function_response=types.FunctionResponse(
                    id=conf.id,
                    name=_CONFIRM_FN,
                    response={"confirmed": True, "payload": {"answer": "us-central1"}},
                )
            )
        ],
    )
    ...
    assert res is not None, "delegate should return a result after the child resumes"
...
async def test_forwarded_parent_grant_skips_the_prompt(monkeypatch) -> None:
    ...
    runner, u, s = await _make_parent(
        state={"permission_grants": [{"toolName": "risky_thing", "decision": "allow"}]}
    )
