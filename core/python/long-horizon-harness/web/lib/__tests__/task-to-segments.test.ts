...
// A2A 1.0 part fixtures. Parts are a ts-proto tagged union under `content`
// and carry required metadata/filename/mediaType, so build them here rather
// than repeating the shape at ~110 call sites.
function tp(text: string): Part {
  return {
...
function dp(data: unknown, metadata?: Record<string, unknown>): Part {
  return {
...
// Fixture helper: fills required Message fields that fixtures omit (contextId,
// taskId, metadata, extensions, referenceTaskIds) and converts string role to
// the numeric 1.0 enum so the cast to Message[] typechecks without widening.
function msg(m: {
  messageId: string;
  role: "user" | "agent";
  parts: Part[];
  ...
  return {
    ...
    contextId: "ctx-1",
    taskId: "task-1",
    role: m.role === "user" ? Role.ROLE_USER : Role.ROLE_AGENT,
    parts: m.parts,
    metadata: m.metadata,
    extensions: [],
    referenceTaskIds: [],
...
function makeTask(history: Message[]): Task {
  return {
    id: "task-1",
    contextId: "ctx-1",
...
describe("taskToChatMessages", () => {
  it("drops adk_partial=true messages so the final aggregated event is rendered once", () => {
    ...
    const task = makeTask([
      msg({ messageId: "p1", role: "agent", metadata: { adk_partial: true }, parts: [tp("I'm doing well, ")] }),
      msg({ messageId: "p2", role: "agent", metadata: { adk_partial: true }, parts: [tp("thank you! ")] }),
      msg({ messageId: "p3", role: "agent", metadata: { adk_partial: true }, parts: [tp("How can I help you today?")] }),
      msg({ messageId: "final", role: "agent", parts: [tp("I'm doing well, thank you! How can I help you today?")] }),
    ]);
    ...
    expect(messages[0].segments).toEqual([
      { kind: "text", text: "I'm doing well, thank you! How can I help you today?" },
    ]);
  });
  ...
  it("attaches a function_response to its function_call across separate history messages", () => {
    ...
    const task = makeTask([
      msg({ messageId: "u1", role: "user", parts: [tp("run ls")] }),
      msg({ messageId: "call", role: "agent", parts: [dp({ id: "call-1", name: "terminal", args: { command: "ls" } }, { adk_type: "function_call" })] }),
      msg({ messageId: "resp", role: "agent", parts: [dp({ name: "terminal", response: { stdout: "a\nb", exit_code: 0 } }, { adk_type: "function_response" })] }),
      msg({ messageId: "final", role: "agent", parts: [tp("Done.")] }),
    ]);
  ...
  it("flushes a pending agent run before a following user message (order preserved)", () => {
    const task = makeTask([
      msg({ messageId: "u1", role: "user", parts: [tp("q1")] }),
      msg({ messageId: "call", role: "agent", parts: [dp({ id: "c1", name: "terminal", args: { command: "ls" } }, { adk_type: "function_call" })] }),
      msg({ messageId: "resp", role: "agent", parts: [dp({ name: "terminal", response: { stdout: "ok", exit_code: 0 } }, { adk_type: "function_response" })] }),
      msg({ messageId: "u2", role: "user", parts: [tp("q2")] }),
      msg({ messageId: "a2", role: "agent", parts: [tp("answer")] }),
    ]);
  ...
  it("attaches each result to its own tool segment when one agent run has two calls", () => {
    const task = makeTask([
      msg({ messageId: "u1", role: "user", parts: [tp("two")] }),
      msg({ messageId: "callA", role: "agent", parts: [dp({ id: "a", name: "terminal", args: { command: "echo a" } }, { adk_type: "function_call" })] }),
      msg({ messageId: "respA", role: "agent", parts: [dp({ id: "a", name: "terminal", response: { stdout: "a", exit_code: 0 } }, { adk_type: "function_response" })] }),
      msg({ messageId: "callB", role: "agent", parts: [dp({ id: "b", name: "terminal", args: { command: "echo b" } }, { adk_type: "function_call" })] }),
      msg({ messageId: "respB", role: "agent", parts: [dp({ id: "b", name: "terminal", response: { stdout: "b", exit_code: 0 } }, { adk_type: "function_response" })] }),
    ]);
  ...
  it("stamps every message with the task id and strictly-increasing createdAt", () => {
    const task = makeTask([
      msg({ messageId: "u1", role: "user", parts: [tp("hi")] }),
      msg({ messageId: "a1", role: "agent", parts: [tp("yo")] }),
    ]);
  ...
  it("keeps non-partial messages untouched", () => {
    const task = makeTask([
      msg({ messageId: "u1", role: "user", parts: [tp("how are you")] }),
      msg({ messageId: "a1", role: "agent", parts: [tp("Doing great.")] }),
    ]);
  ...
  it("folds artifact text into the turn's assistant bubble (new-impl path)", () => {
    ...
    const task = {
      ...
      status: { state: "completed" },
      history: [
        { messageId: "u1", role: "user", parts: [tp("run ls")] },
        {
          messageId: "call",
          role: "agent",
          parts: [
            dp({ id: "c1", name: "terminal", args: { command: "ls" } }, { adk_type: "function_call" }),
          ],
        },
      ],
      artifacts: [
        { artifactId: "art-1", parts: [tp("Here is the listing.")] },
      ],
    } as unknown as Task;
    ...
    expect(assistant.segments).toContainEqual({ kind: "text", text: "Here is the listing." });
  });
  ...
  it("renders a text-only turn from artifacts when history carries no model text", () => {
    const task = {
      ...
      status: { state: "completed" },
      history: [
        { messageId: "u1", role: "user", parts: [tp("hi")] },
      ],
      artifacts: [
        { artifactId: "art-1", parts: [tp("Hello there.")] },
      ],
    } as unknown as Task;
  ...
  it("renders the HITL confirmation card from status.message on an input-required task", () => {
    ...
    const task = {
      ...
      history: [
        { messageId: "u1", role: "user", parts: [tp("run it")] },
        {
          messageId: "call",
          role: "agent",
          parts: [
            dp({ id: "t1", name: "terminal", args: { command: "uv run x" } }, { adk_type: "function_call" }),
          ],
        },
      ],
    } as unknown as Task;
...
describe("taskToChatMessages — HITL resolution on replay", () => {
  it("resolves a confirmation from the user's approve echo (function_response)", () => {
    const task = makeTask([
      msg({ messageId: "u1", role: "user", parts: [tp("run ls")] }),
      msg({ messageId: "conf", role: "agent", parts: [dp({
              id: "adk-conf-2",
              name: "adk_request_confirmation",
              args: {
                originalFunctionCall: { id: "call-terminal-1", name: "terminal", args: { command: "ls /tmp" } },
                toolConfirmation: { hint: "Run shell command: ls /tmp ?", payload: null },
              },
            }, { adk_type: "function_call" })] }),
      msg({ messageId: "echo", role: "user", parts: [dp({
              id: "adk-conf-2",
              name: "adk_request_confirmation",
...
describe("RESIDUAL: HITL-gated tool result attaches across echo / task boundary", () => {
  const call = (id: string, command: string) => (dp({ id, name: "terminal", args: { command } }, { adk_type: "function_call" }));
  const card = (confId: string, origId: string) => (dp({
      id: confId,
      name: "adk_request_confirmation",
      args: {
  ...
  const echo = (confId: string) => (dp({ id: confId, name: "adk_request_confirmation", response: { confirmed: true, payload: null } }, { adk_type: "function_response" }));
  const resp = (id: string) => (dp({ id, name: "terminal", response: { stdout: "done", exit_code: 0 } }, { adk_type: "function_response" }));

  it("attaches the result when the response follows a user approve echo (same task)", () => {
    const task = makeTask([
      msg({ messageId: "u", role: "user", parts: [tp("run it")] }),
      msg({ messageId: "call", role: "agent", parts: [call("c1", "rm -rf x")] }),
      msg({ messageId: "card", role: "agent", parts: [card("conf1", "c1")] }),
      msg({ messageId: "echo", role: "user", parts: [echo("conf1")] }),
      msg({ messageId: "resp", role: "agent", parts: [resp("c1")] }),
    ]);
  ...
  it("attaches the result when the response lands in a LATER task", () => {
    const taskA = {
      id: "tA", contextId: "ctx", status: { state: "completed" },
      history: [{ messageId: "call", role: "agent", parts: [call("c1", "rm -rf x"), card("conf1", "c1")] }],
    } as unknown as Task;
    const taskB = {
      id: "tB", contextId: "ctx", status: { state: "completed" },
      history: [
        { messageId: "echo", role: "user", parts: [echo("conf1")] },
        { messageId: "resp", role: "agent", parts: [resp("c1")] },
      ],
    } as unknown as Task;
