...
def create_frame_with_border(
    width, height, border_color, center_color, border_size=10
):
...
class TestDetectProductBoundsFromFrames:
    ...
    def test_samples_subset_of_frames(self):
        ...
        frames = [
            np.full((100, 100, 3), 255, dtype=np.uint8) for _ in range(100)
        ]
...
class TestProcessFramesToTargetSize:
    ...
    def test_preserves_background_color(self):
        ...
        result = process_frames_to_target_size(
            [frame_bytes], target_size=(300, 300)
        )
