...
def test_agent_runnability() -> None:
    ...
    os.environ.setdefault("MODEL_NAME_GEMINI_2_5_PRO", "gemini-2.5-pro")
    os.environ.setdefault("MODEL_NAME_GEMINI_2_5_FLASH", "gemini-2.5-flash")

    with patch(
        "google.auth.default", return_value=(MagicMock(), "test-project")
    ):
        import airflow_version_upgrade_agent.agent

    assert airflow_version_upgrade_agent.agent.root_agent is not None
