...
const ApproveDiscountInput = z.object({
  discountType: z
    .string()
    .describe("The type of discount, either 'percentage' or 'flat'."),
...
const UpdateSalesforceCrmInput = z.object({
  ...
  details: z
    .object({})
    .passthrough()
    .describe("A dictionary of details to update in Salesforce."),
});
...
const ModifyCartInput = z.object({
  ...
  itemsToAdd: z
    .array(
      z.object({ product_id: z.string(), quantity: z.number() }).passthrough(),
    )
...
const GetProductRecommendationsInput = z.object({
  plantType: z
    .string()
    .describe("The type of plant (e.g., 'Petunias', 'Sun-loving annuals')."),
  customerId: z
    .string()
    .describe("Optional customer ID for personalized recommendations."),
});
...
const CheckProductAvailabilityInput = z.object({
  ...
  storeId: z
    .string()
    .describe("The ID of the store (or 'pickup' for pickup availability)."),
});
...
const SchedulePlantingServiceInput = z.object({
  ...
  details: z
    .string()
    .describe("Any additional details (e.g., 'Planting Petunias')."),
});
...
const GenerateQrCodeInput = z.object({
  ...
  discountValue: z
    .number()
    .describe("The value of the discount (e.g., 10 for 10%)."),
  discountType: z.string().describe("'percentage' (default) or 'fixed'."),
  expirationDays: z
    .number()
    .describe("Number of days until the QR code expires."),
});
