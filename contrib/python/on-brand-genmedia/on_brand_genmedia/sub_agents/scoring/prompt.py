...
1. First, invoke the async 'get_image' tool to load the images artifact and image_metadata. Do not try to generate the image. Wait for the image to be loaded and the response. CRITICAL: If no image artifact was loaded or if the tool returns an error, you MUST immediately return a total_score of 0 with a JSON indicating failure, invoke the set_score tool with 0, and stop evaluation.
...
3. Scoring Criteria: Carefully examine the rules in the JSON string obtained in step 2. Each rule in the policy not only applies to the primary subject of the image but to all content in the image, including backgrounds. Getting the branding right is of utmost importance. For EACH rule described within this JSON string:
    a. Strictly score the loaded image against each criterion mentioned in the JSON string.
    b. Assign a score in a scale of 0 to 5: 5 points if the image complies with a specific criterion, or 0 point if it does not. Also specify the reason in a separate attribute explaining the reason for assigning the score.
