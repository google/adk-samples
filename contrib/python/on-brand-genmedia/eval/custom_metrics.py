...
def name_only_trajectory_evaluator(
    ...
    if not expected_invocations:
        logger.warning(
            "No expected invocations provided for trajectory evaluation."
        )
        return EvaluationResult(overall_eval_status=EvalStatus.NOT_EVALUATED)

    threshold = (
        eval_metric.threshold if eval_metric.threshold is not None else 1.0
    )
    ...
    for actual, expected in zip(
        actual_invocations, expected_invocations, strict=False
    ):
    ...
    overall_score = (
        total_tool_use_accuracy / num_invocations
    ...
    logger.info(
        f"Overall score: {overall_score}, Status: {overall_eval_status}"
    )
