...
| [Intelligent Document Generation Agent](intelligent-document-generation-agent) | Ingests unstructured PDFs, extracts structured data via a multi-agent workflow, generates summary / feature / security Markdown deliverables in parallel, converts to PDF via Cloud Run, and uploads to GCS. | Multi-agent, ParallelAgent, Document extraction, output_schema, Cloud Run, Cloud Storage | Workflow | Intermediate | Multi Agent | Horizontal / Document Processing |
