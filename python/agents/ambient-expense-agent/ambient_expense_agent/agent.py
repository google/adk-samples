...
def auto_approve(node_input: dict) -> Event:
    ...
    error = node_input.get("error")
    if error:
        return Event(
            output={
                "status": "error",
                "message": f"Failed to process expense: {error}",
            }
        )
    ...
    message = f"Expense of ${amount:.2f} from {submitter} has been auto-approved."
    return Event(output={"status": "approved", "message": message, **node_input})
