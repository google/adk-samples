...
class LiveKitGeminiBridge:
    ...
    async def start(self):
        ...
        if self.room and self.audio_track:
            ...
            logger.info(f"Published agent audio track to room: {self.publication.sid if self.publication else 'Unknown'}")
    ...
    async def _read_adk_events(self):
        ...
        while self._running:
            try:
                ...
                # if recent_session and recent_session.events:
                #     logger.info(f"[GEMINI-BIDI-MONITOR] HISTORY_INJECTED - Total Events: {len(recent_session.events)}")
                #     for idx, ev in enumerate(recent_session.events):
                #         text_repr = ""
                ...
                #                     parts_texts.append(f"ToolCall({p.function_call.name})")
                #                 elif getattr(p, 'function_response', None):
                #                     parts_texts.append(f"ToolResponse({p.function_response.name})")
                #             text_repr = " | ".join(parts_texts)
                #         logger.info(f"  [Event {idx+1}] Author: {ev.author} | {text_repr}")
                # else:
                #     logger.info("[GEMINI-BIDI-MONITOR] HISTORY_INJECTED - None (Cold Start)")
                # ==============================================================
                ...
                async for event in self.runner.run_live(
                   ...
                   # if not getattr(event, 'partial', False):
                    if getattr(event, 'input_transcription', None):
                        ...
                        if not getattr(event, 'partial', True):
    ...
    async def stop(self):
        """Stop the bridge."""
        self._running = False
        if self.room and self.publication:
            try:
                logger.info(f"Unpublishing agent audio track: {self.publication.sid}")
                await self.room.local_participant.unpublish_track(self.publication.sid)
            except Exception as ue:
                logger.warning(f"Failed to unpublish track: {ue}")
            self.publication = None
