...
from utils.audio_utils import concatenate_and_resample_audio, find_sorted_output_audio_files
from utils.config import TTS_EVAL_MODEL, TTS_EVAL_GENERATE_CONFIG
...
def main():
    ...
    if not audio_files:
        print("No raw audio files found inside the output directories. Speak to the agent first!")
        return

    print(f"Found {len(audio_files)} raw audio segments. Concatenating and downsampling...")
