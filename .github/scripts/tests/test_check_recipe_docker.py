...
def test_probe_timeout_default_matches_policy():
    """policy.yml deployability.verification.ready_timeout_seconds is 120.

    The default drifted to 90 while policy said 120. Pin them together so the
    next drift is a test failure rather than a silent difference.
    """
    import inspect
    ...
    policy = yaml.safe_load(
        (Path(m.__file__).resolve().parents[1] / "policy.yml").read_text(
            encoding="utf-8"
        )
    )
    expected = policy["deployability"]["verification"]["ready_timeout_seconds"]
    actual = (
        inspect.signature(m.validate_recipe_docker)
        .parameters["probe_timeout"]
        .default
    )
    assert actual == expected, (
        f"validate_recipe_docker probe_timeout default is {actual} but "
        f"policy.yml ready_timeout_seconds is {expected}"
    )
