...
*   [ ] **Design Constraints**: Total directory size **under 1MB**, at most **50 total files**, and demonstrates a clear intent and value for the ADK community.
...
*   **File Limit**: Maximum of **50 total files** (no exceptions, all files are counted).
...
*   **Lowercase only** (lowercase letters and `-` only).
...
### 1.1 Lockfile Management (`uv.lock`)

Every recipe must include a `uv.lock` file to guarantee reproducible runs in all environments.

*   **Create or Update:** Run `uv lock` inside the recipe directory. This command will generate the lockfile if it is missing, or update it after you modify `pyproject.toml` dependencies.
*   **Run from repository root:** You can target a specific recipe without changing directories by using:
    ```bash
    uv lock --project core/<recipe-name>
    ```
