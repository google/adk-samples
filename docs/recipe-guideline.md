...
*   [ ] **Design Constraints**: Total directory size **under 1MB** (excluding `uv.lock`), at most **50 main files** (excluding docs & tests), and demonstrates a clear intent and value for the ADK community.
...
*   [ ] **Required Files**:
    ...
    *   `uv.lock`: lockfile generated by `uv` to pin all dependency versions.
...
*   [ ] **Environment Variables**: `.env.example` must declare at least one variable prefixed with `GOOGLE_CLOUD_PROJECT`, `GOOGLE_CLOUD_LOCATION`, and `MODEL_NAME`.
...
*   **Lowercase only** (lowercase letters and hyphens only).
...
> **Note:** The `extract-python-environment-variables` skill can automate all of the above — scanning your code for env var reads, populating `.env.example`, and injecting the `load_dotenv()` snippet into `__init__.py`. See [Developer Agent Skills](#4-developer-agent-skills-agentsskills) below.
...
To streamline local development, validate coding standards, and automate bootstrapping, we provide helper scripts in `tools/`, integration with **Ruff** for formatting/linting, and agent skills under `.agents/skills/`.

### 1. Code Quality & Formatting (Ruff)
...
### 2. Local Validation Suite (`tools/`)
...
### 3. Running & Testing Recipes Locally
...
### 4. Developer Agent Skills (`.agents/skills/`)
...
*   **Purpose**: Scans all non-test Python files in a recipe for environment variable reads (`os.getenv`, `os.environ.get`, `os.environ[]`), then automatically:
    ...
    *   Detects hardcoded model name strings and replaces them with `os.getenv("MODEL_NAME")` (or `MODEL_NAME_EMBEDDING`, `MODEL_NAME_2`, etc. for multiple models).
    *   Injects the standard `load_dotenv()` bootstrap snippet into `app/__init__.py`.
...
*   **Examples to trigger this skill**:
    ...
    *   > *"Update the .env.example and add load_dotenv to `core/python/rag-agent-search`"*
